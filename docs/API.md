...
* **Authorization** — the guard chain is `RateLimitGuard → JwtAuthGuard → AccountStatusGuard →
  PermissionsGuard`. The table shows the *route-level* gate; object-level checks (is this your
...
* **Idempotency** — routes marked *Idempotency-Key* require that header (8–128 chars). Replays
...
* **Rate limits** — the *Notes* column shows `requests/window-seconds`; exceeding one returns
