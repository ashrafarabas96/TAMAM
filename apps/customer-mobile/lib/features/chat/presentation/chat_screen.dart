...
import 'package:tamam_customer/features/chat/presentation/chat_controller.dart';
...
/// In-job chat with the partner: text, photos and a shared location.
...
class _ChatScreenState extends ConsumerState<ChatScreen> {
  ...
  Future<void> _sendPhoto() async {
    ...
    try {
      ...
      if (uploaded.mediaId == null) return;
      await ref.read(chatProvider(widget.jobId).notifier).sendImage(uploaded.mediaId!);
      _scrollToBottom();
    } on Object catch (error) {
      if (mounted) AppFeedback.showMessage(context, error.toString(), icon: Icons.error_outline_rounded);
    } finally {
  ...
  Future<void> _sendLocation() async {
    ...
    await ref.read(chatProvider(widget.jobId).notifier).sendLocation(point);
    _scrollToBottom();
  }
                ...
                itemBuilder: (BuildContext context, int index) {
                  if (thread.hasMore && index == 0) {
                    return Center(
                      child: TextButton(
                        onPressed: () =>
                            unawaited(ref.read(chatProvider(widget.jobId).notifier).loadOlder()),
        ...
        return Row(
          ...
          children: <Widget>[
            ...
            Text(
              ...
              style: TamamType.bodyMd.toTextStyle(color: foreground),
            ),
          ],
        );
