...
enum AuthStatus {
  ...
  /// Signed in but the profile has no name yet (`isNewUser`), so onboarding
  /// must finish before the customer reaches home.
  needsProfile,
  signedIn,
}
...
class SessionState {
  ...
  /// Set when the session ended without the customer asking (token revoked,
  /// account suspended); the sign-in screen explains why.
  final SignedOutReason? signedOutReason;

  bool get isAuthenticated => status == AuthStatus.signedIn || status == AuthStatus.needsProfile;
