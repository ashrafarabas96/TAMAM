...
export function ConsoleShell({ children }: { children: ReactNode }) {
  ...
  return (
    <div className="flex min-h-screen">
      <aside className={cn('fixed inset-y-0 start-0 z-40 hidden bg-purple-800 transition-[width] duration-base lg:block', collapsed ? 'w-[72px]' : 'w-64')}>
        <Sidebar collapsed={collapsed} />
      </aside>
      {mobileOpen ? (
        <div className="fixed inset-0 z-50 lg:hidden">
          <button type="button" className="absolute inset-0 bg-overlay" onClick={() => setMobileOpen(false)} aria-label={t('common.close')} />
          <aside className="absolute inset-y-0 start-0 w-72 bg-purple-800 shadow-floating animate-slide-in-end">
            <Sidebar collapsed={false} onNavigate={() => setMobileOpen(false)} />
          </aside>
        </div>
      ) : null}
      <div className={cn('flex min-h-screen min-w-0 flex-1 flex-col transition-[margin] duration-base', collapsed ? 'lg:ms-[72px]' : 'lg:ms-64')}>
