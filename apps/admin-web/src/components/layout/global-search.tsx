...
export function GlobalSearch() {
  ...
  const allowed = permissions.canAny(
                ...
                <Group title={t('nav.customers')}>
                  {data.customers.map((c) => (
                    <Row
                      ...
                      secondary={<span dir="ltr">{c.phone}</span>}
                      trailing={<StatusPill group="accountStatus" value={c.accountStatus} />}
                    />
                  ))}
                </Group>
              ) : null}
              {data?.partners && data.partners.length > 0 ? (
                <Group title={t('nav.partners')}>
                  {data.partners.map((p) => (
                    <Row
                      key={p.id}
                      onClick={() => go(`/partners/${p.id}`)}
                      primary={p.fullName ?? p.phone}
                      secondary={<span dir="ltr">{p.phone}</span>}
                      trailing={
                        <>
              ...
              {data?.vehicles && data.vehicles.length > 0 ? (
                <Group title={t('search.vehicles')}>
                  {data.vehicles.map((v) => (
                    <Row
                      key={v.id}
                      onClick={() => go(`/partners/${v.partnerId}?tab=vehicles`)}
                      primary={<span dir="ltr">{v.plate}</span>}
              ...
              {data?.payments && data.payments.length > 0 ? (
                <Group title={t('search.payments')}>
                  {data.payments.map((p) => (
                    <Row
                      key={p.id}
                      onClick={() => go(`/finance/payments/${p.id}`)}
                      primary={formatMinor(p.amountMinor, p.currency, { locale })}
                      secondary={p.providerRef ?? p.id}
                      trailing={<StatusPill group="paymentStatus" value={p.status} />}
                    />
                  ))}
                </Group>
              ) : null}
              {data?.tickets && data.tickets.length > 0 ? (
                <Group title={t('nav.support')}>
                  {data.tickets.map((tk) => (
                    <Row
                      key={tk.id}
                      onClick={() => go(`/support/${tk.id}`)}
                      primary={`${tk.number} · ${tk.subject}`}
                      trailing={<StatusPill group="ticketStatus" value={tk.status} />}
                    />
                  ))}
                </Group>
              ) : null}
              {data?.disputes && data.disputes.length > 0 ? (
                <Group title={t('nav.disputes')}>
                  {data.disputes.map((d) => (
                    <Row
                      key={d.id}
                      onClick={() => go(`/disputes/${d.id}`)}
                      primary={d.number}
                      trailing={<StatusPill group="disputeStatus" value={d.status} />}
                    />
                  ))}
                </Group>
              ) : null}
...
function Group({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <div>
      <p className="px-3 pb-1 pt-2 text-[10px] font-bold uppercase tracking-wider text-text-tertiary">
        {title}
      </p>
      <ul>{children}</ul>
    </div>
  ...
  return (
    <li>
      <button
        type="button"
        onClick={onClick}
        className="flex w-full items-center justify-between gap-3 rounded-md px-3 py-2 text-start text-sm hover:bg-surface-brand-soft"
      >
