  ...
  return (
    <header className="sticky top-0 z-30 flex h-16 items-center gap-3 border-b border-border bg-surface/90 px-4 backdrop-blur">
      ...
      <Tooltip content={collapsed ? t('nav.expandSidebar') : t('nav.collapseSidebar')}>
        ...
        >
          {collapsed ? (
            <PanelLeftOpen className="h-5 w-5 rtl:-scale-x-100" aria-hidden />
          ) : (
            <PanelLeftClose className="h-5 w-5 rtl:-scale-x-100" aria-hidden />
          )}
        </Button>
      </Tooltip>
      ...
      <div className="flex items-center gap-1">
        ...
        <DropdownMenu
          ...
          trigger={
            <button
              type="button"
              className="ms-1 flex items-center gap-2 rounded-pill p-1 pe-3 hover:bg-surface-alt"
              aria-label={t('nav.staffMenu')}
            >
              <Avatar
              ...
              <span className="hidden text-start md:block">
                <span className="block max-w-[160px] truncate text-xs font-semibold text-text-primary">
          ...
          items={[
            {
              key: 'account',
              label: <Link href="/account">{t('nav.account')}</Link>,
              icon: <UserRound />,
              href: '/account',
            },
            {
