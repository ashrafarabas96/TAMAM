...
export function JobsList({
  ...
  fixed?: { customerId?: string; partnerId?: string };
  ...
  const [statusGroup, setStatusGroup] = useState<'active' | 'completed' | 'cancelled' | 'all'>(
    'all',
  );
  ...
  const filters = useMemo(
    () => ({
      q: q || undefined,
      status: (status || undefined) as JobDto['status'] | undefined,
      statusGroup,
      type: (type || undefined) as JobDto['type'] | undefined,
      zoneId: zoneId || undefined,
      from: fromDateTimeLocalValue(from) ?? undefined,
      to: fromDateTimeLocalValue(to) ?? undefined,
      ...fixed,
    }),
    [q, status, statusGroup, type, zoneId, from, to, fixed],
  );
  const list = useCursorList<JobDto>({
    queryKey: queryKeys.jobs.list(filters),
    fetchPage: (cursor) => jobsApi.list({ ...filters, cursor, limit: 30 }),
  });

  const columns: Column<JobDto>[] = [
    {
      key: 'number',
      header: t('jobs.number'),
      cell: (j) => (
        <Link
          href={`/jobs/${j.id}`}
    ...
    },
    {
      ...
      cell: (j) => <StatusPill group="jobType" value={j.type} />,
    },
    {
      key: 'status',
      header: t('common.status'),
      cell: (j) => <StatusPill group="jobStatus" value={j.status} />,
    },
    { key: 'zone', header: t('common.zone'), cell: (j) => zones.nameOf(j.zoneId) },
    {
      ...
      cell: (j) => (
        <span className="line-clamp-2 max-w-[240px] text-xs text-text-secondary">
          {j.stops[0]?.address.formatted ?? '—'}
        </span>
      ),
    },
    {
      key: 'customer',
      header: t('jobs.customer'),
      cell: (j) => j.customer?.fullName ?? j.customerId.slice(0, 8),
    },
    {
      key: 'partner',
    ...
    },
    {
      ...
      align: 'end',
      cell: (j) => <Money value={j.finalTotal ?? j.estimatedTotal} />,
    },
    {
      key: 'payment',
      header: t('jobs.paymentMethod'),
      cell: (j) => <StatusPill group="paymentMethod" value={j.paymentMethod} />,
    },
    {
      key: 'created',
  ...
  return (
    <div>
      <FilterBar>
        ...
        <Select
          value={statusGroup}
          onValueChange={(v) => setStatusGroup(v as 'all')}
          options={groups}
          aria-label={t('jobs.statusGroup')}
        />
        <Select
          value={status}
