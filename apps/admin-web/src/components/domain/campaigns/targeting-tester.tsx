...
export function TargetingTester({ highlightCampaignId }: { highlightCampaignId?: string }) {
  ...
  const [input, setInput] = useState<CampaignPreviewInput>({
  ...
  const preview = useMutation({
    mutationFn: () => campaignsApi.preview(input),
    ...
    <Card title={t('campaigns.tester')} description={t('campaigns.testerHint')}>
      <div className="grid gap-4 lg:grid-cols-[320px_1fr]">
        <div className="space-y-3">
          <div>
            ...
            <NativeSelect
              ...
              onChange={(e) =>
                setInput((p) => ({ ...p, placement: e.target.value as BannerPlacement }))
              }
            >
              {Object.values(BannerPlacement).map((pl) => (
                <option key={pl} value={pl}>
                  {enumLabel('bannerPlacement', pl)}
                </option>
              ))}
            </NativeSelect>
          </div>
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <NativeSelect
                ...
                onChange={(e) =>
                  setInput((p) => ({ ...p, audience: e.target.value as 'CUSTOMER' }))
                }
              >
            ...
            <div>
              ...
              <NativeSelect
                ...
                onChange={(e) =>
                  setInput((p) => ({ ...p, language: e.target.value as 'ar' | 'en' }))
                }
              >
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <NativeSelect
                id="tester-zone"
                value={input.zoneId ?? ''}
                onChange={(e) => setInput((p) => ({ ...p, zoneId: e.target.value || undefined }))}
              >
                {zones.options.map((z) => (
            ...
            <div>
              ...
              <NativeSelect
                ...
                onChange={(e) =>
                  setInput((p) => ({
                    ...p,
                    platform: (e.target.value || undefined) as 'ios' | undefined,
                  }))
                }
              >
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <Input
                ...
                min={0}
                dir="ltr"
                value={input.completedJobs ?? 0}
                onChange={(e) =>
                  setInput((p) => ({ ...p, completedJobs: Number(e.target.value) || 0 }))
                }
              />
            </div>
            <div>
              ...
              <Input
                ...
                placeholder="uuid"
                value={input.userId ?? ''}
                onChange={(e) => setInput((p) => ({ ...p, userId: e.target.value || undefined }))}
              />
            </div>
          </div>
          <Checkbox
            checked={!!input.isNewCustomer}
            onCheckedChange={(v) => setInput((p) => ({ ...p, isNewCustomer: v }))}
            label={t('campaigns.isNewCustomer')}
          />
          <div>
              ...
              {[JobType.RIDE, JobType.DELIVERY, JobType.HOME_SERVICE].map((j) => (
                <Checkbox
                  ...
                  onCheckedChange={(checked) =>
                    setInput((p) => ({
                      ...p,
                      usedJobTypes: checked
                        ? [...(p.usedJobTypes ?? []), j]
                        : (p.usedJobTypes ?? []).filter((x) => x !== j),
                    }))
                  }
        ...
        <div>
          {!preview.data ? (
            <EmptyState
              title={t('campaigns.testerEmpty')}
              description={t('campaigns.testerEmptyHint')}
            />
          ) : preview.data.length === 0 ? (
            <EmptyState
              title={t('campaigns.testerNoMatch')}
              description={t('campaigns.testerNoMatchHint')}
            />
          ) : (
            <div className="grid gap-4 md:grid-cols-2">
              {preview.data.map((banner) => (
                <div
                  key={banner.id}
                  className={
                    highlightCampaignId && banner.campaignId === highlightCampaignId
                ...
                >
                  <PhoneFrame>
                    <BannerPreview
                      ...
                      value={{
                        ...
                        imageUrl:
                          input.language === 'en'
                            ? banner.creative.imageUrl.en
                            : banner.creative.imageUrl.ar,
                      }}
                    />
                  </PhoneFrame>
                  <p className="mt-2 flex flex-wrap justify-center gap-1 text-[11px]">
                    ...
                    {highlightCampaignId && banner.campaignId === highlightCampaignId ? (
                      <Badge tone="accent">{t('campaigns.thisCampaign')}</Badge>
                    ) : null}
                  </p>
                </div>
              ))}
