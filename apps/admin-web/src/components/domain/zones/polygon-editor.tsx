...
export function PolygonEditor({
  ...
  const emit = useCallback(
    (points: Position[]) => {
      ...
      onChange({
        type: 'Polygon',
        coordinates: [[...points, [first[0] as number, first[1] as number]]],
      });
    },
    [onChange],
  );
  ...
  const render = useCallback(() => {
    ...
    if (points.length >= 2)
      features.push({
        type: 'Feature',
        geometry: { type: 'LineString', coordinates: [...points, points[0] as Position] },
        properties: {},
      });
    if (points.length >= 3)
      features.push({
        type: 'Feature',
        geometry: { type: 'Polygon', coordinates: [[...points, points[0] as Position]] },
        properties: {},
      });
  ...
  useEffect(() => {
    ...
    if (!map.getSource(SRC)) {
      ...
      map.addLayer({
        ...
        paint: { 'fill-color': tokens.color.brand.purple[500], 'fill-opacity': 0.12 },
      });
      map.addLayer({
        id: 'draft-line',
        type: 'line',
        ...
        paint: { 'line-color': tokens.color.brand.purple[500], 'line-width': 2 },
      });
      map.addLayer({
        id: 'draft-points',
        type: 'circle',
        ...
        paint: {
          'circle-radius': 6,
          'circle-color': tokens.color.brand.yellow[500],
          'circle-stroke-color': tokens.color.brand.purple[700],
  ...
  return (
    <div className={cn('relative overflow-hidden rounded-lg border border-border', className)}>
      <div
        ...
        role="application"
        aria-label={t('zones.polygonEditor')}
      />
      {!readOnly ? (
        <div className="absolute end-3 top-3 z-10 flex flex-col gap-2 rounded-md bg-surface/95 p-2 shadow-raised">
          ...
          <Button
            size="sm"
            variant="outline"
            onClick={() => emit(pointsRef.current.slice(0, -1))}
            disabled={count === 0}
          >
          ...
          <p className="text-center text-[11px] text-text-secondary">
            {t('zones.vertices', { count })}
          </p>
        </div>
      ) : null}
      {mode === 'draw' && !readOnly ? (
        <p className="absolute bottom-3 start-3 z-10 rounded-sm bg-surface/95 px-2 py-1 text-[11px] text-text-secondary shadow-card">
          {t('zones.drawHint')}
        </p>
      ) : null}
    </div>
