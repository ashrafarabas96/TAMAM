...
export function DocumentsPanel({
  ...
  if (documents.length === 0)
    return <p className="text-xs text-text-secondary">{t('partners.noDocuments')}</p>;
  return (
    <>
        ...
        {documents.map((doc) => (
          <li key={doc.id} className="card overflow-hidden">
            <div className="flex h-40 items-center justify-center bg-surface-alt">
              {/^https?:\/\//.test(doc.fileUrl) ? (
                <img
                  src={doc.fileUrl}
                  alt={enumLabel('documentType', doc.type)}
                  className="h-full w-full object-contain"
                />
              ) : (
                <FileText className="h-10 w-10 text-text-tertiary" aria-hidden />
              )}
            </div>
            <div className="space-y-2 p-3 text-sm">
              ...
              <p className="text-xs text-text-secondary">
                ...
                {doc.expiresAt ? (
                  <>
                    {t('partners.expiresAt')} <DateTime value={doc.expiresAt} mode="date" />
                  </>
                ) : (
                  t('partners.noExpiry')
                )}
              ...
              {doc.verifiedAt ? (
                <p className="text-[11px] text-text-tertiary">
                  {t('partners.reviewedAt')} <DateTime value={doc.verifiedAt} />
                </p>
              ) : null}
              <div className="flex gap-2">
                <a
                  ...
                  target="_blank"
                  rel="noreferrer"
                  className="inline-flex h-8 items-center gap-1 rounded-button border border-border px-2 text-xs hover:bg-surface-alt"
                >
                  <ExternalLink className="h-3.5 w-3.5" aria-hidden /> {t('common.open')}
                </a>
                {canReview && doc.status === 'PENDING' ? (
                  <Button size="sm" onClick={() => setActive(doc)}>
                    {t('partners.review')}
                  </Button>
                ) : null}
              </div>
...
export function ReviewDialog({
  ...
  doc: { id: string; type?: string } | null;
  onClose: () => void;
  onReview: DocumentReviewSubmit;
  onDone: () => Promise<unknown> | void;
  title?: string;
  ...
  const form = useForm<ReviewDocumentInput>({
  ...
  return (
    <Dialog
      ...
      title={title ?? t('partners.reviewDocument')}
      size="sm"
      ...
      <form onSubmit={submit} className="space-y-3">
        ...
        {decision === 'APPROVE' ? (
          <TextField
            control={form.control}
            name="expiresAt"
            label={t('partners.expiresAt')}
            type="date"
            dir="ltr"
            hint={t('partners.expiresHint')}
          />
        ) : (
          <TextareaField
            control={form.control}
            name="rejectionReason"
            label={t('partners.rejectionReason')}
            required
        ...
        {doc ? (
          <p className="text-xs text-text-tertiary">
            <Badge tone="neutral">{doc.id.slice(0, 8)}</Badge>
          </p>
        ) : null}
      </form>
    </Dialog>
