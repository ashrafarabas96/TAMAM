...
const ROLE_FOR_TYPE: Record<string, 'DRIVER' | 'COURIER' | 'TECHNICIAN' | undefined> = {
  RIDE: 'DRIVER',
  DELIVERY: 'COURIER',
  HOME_SERVICE: 'TECHNICIAN',
};
...
export function AssignSheet({ job, onClose }: { job: JobDto | null; onClose: () => void }) {
  ...
  const nearby = useQuery({
    ...
    queryFn: () =>
      dispatchApi.nearbyPartners({
        lat: stop?.address.lat ?? 0,
        lng: stop?.address.lng ?? 0,
        radiusMeters: radius,
        jobId: job?.id,
        ...(role ? { role: role as 'DRIVER' } : {}),
        limit: 30,
      }),
    enabled: !!job && !!stop,
  });
  ...
  const assign = useMutation({
    mutationFn: (input: { partnerId: string; reason: string }) =>
      dispatchApi.assign(job?.id ?? '', {
  ...
  return (
    <Sheet
      ...
      title={t('dispatch.manualAssign')}
      description={job ? `${job.number} · ${stop?.address.formatted ?? ''}` : undefined}
      width="lg"
    >
          ...
          <NativeSelect id="role" value={role} onChange={(e) => setRole(e.target.value)}>
            ...
            {['DRIVER', 'COURIER', 'TECHNICIAN', 'SERVICE_PROVIDER'].map((r) => (
              <option key={r} value={r}>
                {t(`enum.partnerRole.${r}` as 'enum.partnerRole.DRIVER')}
              </option>
            ))}
          </NativeSelect>
        </div>
            ...
            >
              <div className="min-w-0 text-sm">
                ...
                <p className="flex flex-wrap items-center gap-2 text-xs text-text-secondary">
                  ...
                  <span>
                    {t('partners.completedJobs')}: {p.completedJobs}
                  </span>
              ...
              <Button
                size="sm"
                onClick={() => setCandidate({ id: p.partnerId, name: p.fullName ?? p.phone })}
                disabled={p.availability !== 'ONLINE' && p.availability !== 'BUSY'}
              >
                {t('dispatch.assign')}
              </Button>
            </li>
      ...
      <ConfirmDialog
        ...
        description={
          candidate
            ? t('dispatch.confirmAssignDescription', {
        ...
        onConfirm={(reason) => {
          if (candidate) assign.mutate({ partnerId: candidate.id, reason });
        }}
      />
    </Sheet>
  );
