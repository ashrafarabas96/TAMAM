...
export function Money({
  ...
  value: MoneyValue | { amount: number; currency: string } | null | undefined;
  signed?: boolean;
  className?: string;
}) {
  ...
  return (
    <span className={cn('tabular font-semibold', negative && 'text-danger', className)} dir="ltr">
      {formatMoney(value, { locale, ...(signed ? { signed } : {}) })}
    </span>
  );
}
  ...
  return (
    <span
      className={cn(
        ...
        numeric !== null && numeric < 0 && 'text-danger',
        className,
      )}
      dir="ltr"
    >
      {formatMinor(numeric, currency, { locale, ...(signed ? { signed } : {}) })}
    </span>
  );
}
