...
export const dispatchApi = {
  nearbyPartners: (query: Partial<NearbyPartnersQueryInput> & { lat: number; lng: number }) => api.get<NearbyPartnerDto[]>('/admin/dispatch/nearby-partners', { ...query }),
  assignments: (jobId: string) => api.get<JobAssignmentRow[]>(`/admin/jobs/${jobId}/assignments`),
  assign: (jobId: string, input: ManualAssignInput) => api.post<JobDto>(`/admin/jobs/${jobId}/assign`, input),
