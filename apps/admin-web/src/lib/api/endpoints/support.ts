...
export const supportApi = {
  list: (query: {
    ...
    q?: string;
  }) => api.get<Page<SupportTicketDto>>('/admin/support/tickets', query),
  get: (id: string) => api.get<SupportTicketDetailDto>(`/admin/support/tickets/${id}`),
  update: (id: string, input: UpdateTicketInput) =>
    api.patch<SupportTicketDto>(`/admin/support/tickets/${id}`, input),
  addMessage: (id: string, input: TicketMessageInput) =>
    api.post<SupportMessageDto>(`/admin/support/tickets/${id}/messages`, input),
  reports: (query: {
    cursor?: string;
    limit?: number;
    status?: string;
    ...
    reporterId?: string;
    jobId?: string;
  }) => api.get<Page<UserReportDto>>('/admin/support/reports', query),
};
