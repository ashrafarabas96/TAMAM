...
describe('api client', () => {
  ...
  it('sends bearer, request id, language and serialises the query string', async () => {
    ...
    expect(call.url).toBe(
      'https://api.test/api/v1/admin/jobs?status=SEARCHING&limit=20&tags=a&tags=b',
    );
  ...
  it('passes the Idempotency-Key header through', async () => {
    const client = createApiClient({
      ...
      getAccessToken: () => 't',
      refreshAccessToken: async () => null,
      fetchImpl: record(() => jsonResponse({ ok: true })),
    });
    await client.post(
      '/admin/refunds',
      { amountMinor: 100 },
      { idempotencyKey: 'refund-123456789' },
    );
  ...
  it('refreshes once on 401 and retries the request', async () => {
    ...
    const client = createApiClient({
      ...
      fetchImpl: record((call) =>
        (call.init.headers as Record<string, string>).Authorization === 'Bearer token-1'
  ...
  it('single-flights the refresh when several requests fail at once', async () => {
    ...
    const client = createApiClient({
      ...
      refreshAccessToken: refresh,
      fetchImpl: record((call) =>
        (call.init.headers as Record<string, string>).Authorization === 'Bearer stale'
  ...
  it('reports unauthenticated when the refresh fails', async () => {
    ...
    const client = createApiClient({
      ...
      onUnauthenticated,
      fetchImpl: record(() =>
        jsonResponse({ code: 'TOKEN_REVOKED', message: 'gone', requestId: 'req-1' }, 401),
      ),
    });
  ...
  it('does not refresh for anonymous requests', async () => {
    ...
    const client = createApiClient({
      ...
      getAccessToken: () => null,
      refreshAccessToken: refresh,
      fetchImpl: record(() => jsonResponse({ code: 'UNAUTHENTICATED' }, 401)),
    });
    await expect(
      client.get('/config/feature-flags', undefined, { anonymous: true }),
