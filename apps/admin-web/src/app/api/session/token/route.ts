...
export async function GET(request: Request): Promise<Response> {
  ...
  if (!session)
    return NextResponse.json(
      { code: 'UNAUTHENTICATED', message: 'No session', requestId: 'admin-web' },
      { status: 401 },
    );
  if (new Date(session.refreshExpiresAt).getTime() <= Date.now()) {
    clearSessionCookie();
    return NextResponse.json(
      { code: 'TOKEN_EXPIRED', message: 'Session expired', requestId: 'admin-web' },
      { status: 401 },
    );
  }
  ...
  if (!renewed) {
    clearSessionCookie();
    return NextResponse.json(
      { code: 'TOKEN_REVOKED', message: 'Session could not be renewed', requestId: 'admin-web' },
      { status: 401 },
    );
  }
