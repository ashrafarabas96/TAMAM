...
function DisputeDetail() {
  ...
  const messageForm = useForm<DisputeMessageInput>({
    resolver: zodResolver(disputeMessageSchema),
    defaultValues: { text: '', evidenceMediaIds: [], internal: false },
  });
  const decisionForm = useForm<DecideDisputeInput>({
    resolver: zodResolver(decideDisputeSchema),
    defaultValues: {
  ...
  if (dispute.isError)
    return <ErrorState error={dispute.error} onRetry={() => void dispute.refetch()} />;
  ...
  return (
    <div className="space-y-5">
      <PageHeader
        ...
        actions={
          <Can anyOf={[Permission.DISPUTES_DECIDE]}>
            <Button
              variant="accent"
              size="sm"
              ...
              disabled={decided}
            >
              {decided ? t('disputes.alreadyDecided') : t('disputes.decide')}
            </Button>
          </Can>
        }
      />
        ...
        <Card title={t('disputes.case')} className="lg:col-span-2">
            ...
            <div className="mt-3 flex flex-wrap gap-2">
              {d.evidenceUrls.map((url) => (
                <a
                  ...
                  target="_blank"
                  rel="noreferrer"
                  className="block h-20 w-28 overflow-hidden rounded-sm border border-border"
                >
                  <img
                    src={url}
                    alt={t('disputes.evidence')}
          ...
          <ul className="mt-4 space-y-2 border-t border-border pt-3">
            {d.messages.map((m) => (
              <li
                key={m.id}
                className={`rounded-md p-3 text-sm ${m.internal ? 'bg-warning-soft/40' : 'bg-surface-alt'}`}
              >
            ...
            items={[
              {
                label: t('jobs.job'),
                value: (
                  <Link className="text-primary hover:underline" href={`/jobs/${d.jobId}`}>
                    <Identifier value={d.jobId} />
                  </Link>
                ),
              },
              {
              ...
              },
              {
                ...
                value: (
                  <Link className="text-primary hover:underline" href={`/partners/${d.partnerId}`}>
                    <Identifier value={d.partnerId} />
                  </Link>
                ),
              },
              ...
              {
                label: t('disputes.partnerAdjustment'),
                value: <Money value={d.partnerAdjustment} signed />,
              },
      ...
      <Dialog
        ...
        footer={
          <>
            <Button variant="ghost" onClick={() => setDecideOpen(false)}>
              {t('common.cancel')}
            </Button>
            <Button
              variant="accent"
              loading={decide.isPending}
              onClick={decisionForm.handleSubmit((v) => decide.mutate(v))}
            >
      ...
      >
        <form className="space-y-4" onSubmit={decisionForm.handleSubmit((v) => decide.mutate(v))}>
          <NativeSelectField
            ...
            name="decision"
            label={t('disputes.decision')}
            options={DECISIONS.map((dec) => ({
              value: dec,
              label: enumLabel('disputeDecision', dec),
            }))}
            required
          />
          <FormGrid cols={2}>
            <MinorAmountField
              control={decisionForm.control}
              name="refundMinor"
              label={t('disputes.refund')}
              currency={currency}
              hint={t('disputes.refundHint')}
            />
            <NumberField
              control={decisionForm.control}
              name="partnerAdjustmentMinor"
              label={t('disputes.partnerAdjustmentMinor')}
              hint={t('disputes.partnerAdjustmentHint')}
            />
          </FormGrid>
          <TextareaField
            control={decisionForm.control}
