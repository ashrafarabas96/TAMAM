      ...
      <PageHeader
        ...
        actions={
          <Can anyOf={[Permission.CAMPAIGNS_MANAGE]}>
            <Link
              href="/campaigns/new"
              className="inline-flex h-9 items-center gap-2 rounded-button bg-primary px-3 text-xs font-semibold text-text-on-brand hover:bg-primary-hover"
            >
...
function CampaignsList() {
  ...
  const list = useCursorList<CampaignDto>({
    ...
    fetchPage: (cursor) => campaignsApi.list({ ...filters, cursor, limit: 30 }),
  });
  ...
  const columns: Column<CampaignDto>[] = [
    {
      ...
      cell: (c) => (
        <span>
          ...
          <span className="block text-xs text-text-secondary">{c.description ?? ''}</span>
        </span>
      ),
    },
    {
      ...
      cell: (c) => <StatusPill group="campaignStatus" value={c.status} />,
    },
    {
      key: 'placements',
      header: t('campaigns.placements'),
      cell: (c) => (
        <span className="flex flex-wrap gap-1">
          {[...new Set(c.banners.map((b) => b.placement))].map((p) => (
    ...
    },
    {
      ...
      cell: (c) => (
        <span className="text-xs">
          <DateTime value={c.startsAt} mode="date" />
          {c.endsAt ? (
            <>
              {' '}
              → <DateTime value={c.endsAt} mode="date" />
            </>
          ) : (
            ` → ${t('campaigns.openEnded')}`
          )}
        </span>
      ),
    },
    {
      ...
      align: 'end',
      cell: (c) => <span className="tabular">{formatNumber(c.stats.impressions, locale)}</span>,
    },
    {
      key: 'clicks',
      header: t('campaigns.clicks'),
      align: 'end',
      cell: (c) => <span className="tabular">{formatNumber(c.stats.clicks, locale)}</span>,
    },
    {
      key: 'ctr',
      header: t('campaigns.ctr'),
      align: 'end',
      cell: (c) => (
        <span className="tabular font-semibold">{formatPercent(c.stats.ctr, locale, 2)}</span>
      ),
    },
    {
      key: 'conversions',
      ...
      align: 'end',
      cell: (c) => <span className="tabular">{formatNumber(c.stats.conversions, locale)}</span>,
    },
    {
      key: 'rollout',
      header: t('campaigns.rolloutPercent'),
      align: 'end',
      cell: (c) => `${c.targeting.rolloutPercent}%`,
    },
    {
      key: 'updated',
      header: t('common.updatedAt'),
      cell: (c) => <DateTime value={c.updatedAt} mode="relative" />,
    },
  ];
  ...
  return (
    <div>
      ...
      <DataTable
        ...
        emptyDescription={t('campaigns.emptyDescription')}
      />
    </div>
  );
}
