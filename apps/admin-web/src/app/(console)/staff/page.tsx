        ...
        items={[
          {
            ...
            label: t('staff.users'),
            content: (
              <Can anyOf={[Permission.ADMIN_USERS_MANAGE]}>
                <StaffUsersTab />
              </Can>
            ),
          },
          {
            ...
            label: t('staff.roles'),
            content: (
              <Can anyOf={[Permission.ROLES_MANAGE]}>
                <RolesMatrixTab />
              </Can>
            ),
          },
...
function StaffUsersTab() {
  ...
  const list = useCursorList<StaffUserDto>({
    queryKey: queryKeys.staff.list(filters),
    fetchPage: (cursor) => adminApi.staffList({ ...filters, cursor, limit: 30 }),
  });
  const invalidate = () => queryClient.invalidateQueries({ queryKey: queryKeys.staff.all });

  const reset = useMutation({
    mutationFn: (input: { id: string; reason: string }) =>
      adminApi.staffResetPassword(input.id, input.reason),
    onSuccess: async (result) => {
      setTemporaryPassword(result.temporaryPassword);
      setResetFor(null);
      toast.success(
        t('staff.passwordReset'),
        t('staff.sessionsRevoked', { count: result.revokedSessions }),
      );
      await invalidate();
    },
  ...
  const columns: Column<StaffUserDto>[] = [
    {
      ...
      cell: (s) => (
        <span className="flex items-center gap-2">
          <Avatar name={s.user.fullName} src={s.user.profileImageUrl} size="sm" />
          <span>
    ...
    },
    {
      ...
      cell: (s) => <StatusPill group="accountStatus" value={s.user.accountStatus} />,
    },
    {
      key: 'mustChange',
      header: t('staff.mustChangePassword'),
      cell: (s) =>
        s.mustChangePassword ? (
          <Badge tone="warning">{t('common.yes')}</Badge>
        ) : (
    ...
    {
      key: 'locked',
      header: t('staff.lockedUntil'),
      cell: (s) => <DateTime value={s.lockedUntil} />,
    },
    {
      key: 'lastLogin',
      header: t('staff.lastLogin'),
      cell: (s) => <DateTime value={s.lastLoginAt} mode="relative" />,
    },
    { key: 'id', header: t('common.id'), cell: (s) => <Identifier value={s.user.id} /> },
    {
      ...
      cell: (s) => (
        <div className="flex justify-end gap-1">
          <Can anyOf={[Permission.ROLES_MANAGE]}>
            <Button size="sm" variant="ghost" onClick={() => setRolesFor(s)}>
              {t('staff.editRoles')}
            </Button>
          </Can>
          ...
          <Button size="sm" variant="danger-soft" onClick={() => setStatusFor(s)}>
  ...
  return (
    <div>
      ...
      <CreateStaffDialog
        ...
        onCreated={(password) => setTemporaryPassword(password)}
      />
      ...
      {statusFor ? (
        <AccountStatusDialog
          ...
          subject={statusFor.user.fullName ?? statusFor.email ?? ''}
          submit={(input) => adminApi.staffChangeStatus(statusFor.user.id, input)}
          onDone={invalidate}
        />
      ) : null}
      <ConfirmDialog
        ...
        description={t('staff.resetHint')}
        requireReason
        loading={reset.isPending}
        tone="danger"
        onConfirm={(reason) => {
          if (resetFor) reset.mutate({ id: resetFor.user.id, reason });
        }}
      />
      <Dialog
        open={!!temporaryPassword}
        onOpenChange={(o) => !o && setTemporaryPassword(null)}
        size="sm"
        title={t('staff.temporaryPassword')}
        description={t('staff.temporaryPasswordHint')}
        footer={<Button onClick={() => setTemporaryPassword(null)}>{t('common.close')}</Button>}
      >
        <p
          className="select-all rounded-md bg-surface-alt p-3 text-center font-mono text-lg font-bold"
          dir="ltr"
        >
          {temporaryPassword}
        </p>
...
function CreateStaffDialog({
  ...
  const form = useForm<CreateAdminUserInput>({
    ...
    defaultValues: { email: '', fullName: '', roles: [UserRole.SUPPORT], temporaryPassword: '' },
  });
  useEffect(() => {
    if (open)
      form.reset({ email: '', fullName: '', roles: [UserRole.SUPPORT], temporaryPassword: '' });
  }, [open, form]);
  ...
  return (
    <Dialog
      ...
      title={t('staff.newUser')}
      description={t('staff.newUserHint')}
    ...
    >
      <form onSubmit={submit} className="space-y-4">
        ...
        <CheckboxGroupField
          ...
          name="roles"
          label={t('staff.roles')}
          options={ADMIN_ROLES.map((r) => ({ value: r, label: enumLabel('userRole', r) }))}
          required
        />
      </form>
    </Dialog>
...
function RolesDialog({ staff, onClose }: { staff: StaffUserDto | null; onClose: () => void }) {
  ...
  const toast = useToast();
  const queryClient = useQueryClient();
  const form = useForm<UpdateAdminRolesInput>({
    resolver: zodResolver(updateAdminRolesSchema),
    defaultValues: { roles: [], reason: '' },
  });
  useEffect(() => {
    if (staff)
      form.reset({
        roles: staff.user.roles.filter((r) => (ADMIN_ROLES as readonly string[]).includes(r)),
        reason: '',
      });
  }, [staff, form]);
  const mutation = useMutation({
    mutationFn: (input: UpdateAdminRolesInput) =>
      adminApi.staffUpdateRoles(staff?.user.id ?? '', input),
    onSuccess: async () => {
  ...
  return (
    <Dialog
      ...
      title={t('staff.editRoles')}
      description={staff?.user.fullName ?? staff?.email ?? undefined}
      locked={mutation.isPending}
      footer={
        <>
...
function RolesMatrixTab() {
  ...
  const permissions = useQuery({
    queryKey: queryKeys.staff.permissions,
    queryFn: rbacApi.permissions,
  });
  const [editing, setEditing] = useState<RoleDto | null>(null);
  const [selected, setSelected] = useState<Set<string>>(new Set());
  const form = useForm<UpsertRoleInput>({
    resolver: zodResolver(upsertRoleSchema),
    defaultValues: { name: '', permissions: [], reason: '' },
  });
  ...
  const catalog =
    ...
    ALL_PERMISSIONS.map((key) => ({
      key,
      sensitive: (SENSITIVE_PERMISSIONS as readonly string[]).includes(key),
    }));
                ...
                {roles.data.map((role) => (
                  <th key={role.id} className="px-3 py-2 text-center">
                    ...
                    <span className="block text-[10px] font-normal text-text-tertiary">
                      {t('staff.userCount', { count: role.userCount })}
                    </span>
                  </th>
                ))}
              </tr>
              ...
              {groups.map(([group, entries]) => (
                <Fragment key={group}>
                  <tr className="bg-surface-alt/60">
                    <td
                      className="px-4 py-1.5 text-xs font-bold text-text-secondary"
                      ...
                      {roles.data.map((role) => (
                        <td key={role.id} className="px-3 py-2 text-center">
                          ...
                          role.permissions.includes(entry.key) ? (
                            ...
                            <X
                              className="mx-auto h-4 w-4 text-text-tertiary"
          ...
          <>
            ...
            <Button
              ...
              onClick={form.handleSubmit((v) =>
                mutation.mutate({ ...v, permissions: [...selected] }),
              )}
            >
              {t('common.save')}
            </Button>
