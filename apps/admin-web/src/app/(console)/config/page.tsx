...
function SystemConfigTab() {
  ...
  const mutation = useMutation({
    mutationFn: (input: { key: string; value: number | string | boolean; reason: string }) =>
      configApi.update(input),
    onSuccess: async () => {
    ...
    <div className="space-y-4">
      <FilterBar>
        ...
        <NativeSelect
          ...
          aria-label={t('config.group')}
          className="max-w-[220px]"
        >
          <option value="">{t('common.all')}</option>
          {groups.map((g) => (
            <option key={g} value={g}>
              {enumLabel('configGroup', g)}
            </option>
          ))}
        </NativeSelect>
      </FilterBar>
      ...
      {grouped.map(([groupName, rows]) => (
        <Card
          key={groupName}
          title={enumLabel('configGroup', groupName)}
          description={t('config.groupHint', { count: rows.length })}
        >
          <ul className="divide-y divide-border">
            {rows.map((row) => (
              <ConfigRow
                key={row.key}
                config={row}
                onChange={(value) => setPending({ config: row, value })}
              />
            ))}
          </ul>
        </Card>
      ...
      <ConfirmDialog
        ...
        description={
          pending
            ? `${pending.config.key}: ${String(pending.config.value)} → ${String(pending.value)}`
            : undefined
        }
        requireReason
        reasonMinLength={3}
        ...
        onConfirm={(reason) => {
          if (pending) mutation.mutate({ key: pending.config.key, value: pending.value, reason });
        }}
      />
    </div>
  );
  ...
  return (
    <li className="flex flex-wrap items-center gap-3 py-3">
      <div className="min-w-0 flex-1">
        <p className="font-mono text-xs font-semibold text-text-primary" dir="ltr">
          {config.key}
        </p>
          ...
          <>
            ...
            <Can anyOf={[Permission.CONFIG_MANAGE]}>
              <Button
                size="sm"
                disabled={!dirty}
                onClick={() => onChange(config.type === 'number' ? Number(draft) : draft)}
              >
                {t('common.save')}
              </Button>
            </Can>
...
function FeatureFlagsTab() {
  ...
  const mutation = useMutation({
    mutationFn: (input: { key: string; enabled: boolean; reason: string }) =>
      configApi.updateFlag(input.key, { enabled: input.enabled, reason: input.reason }),
    onSuccess: async () => {
  ...
  return (
    <div className="space-y-3">
          ...
          <li key={flag.key} className="flex flex-wrap items-center gap-3 py-3">
            <div className="min-w-0 flex-1">
              ...
              {flag.rollout ? (
                <p className="mt-1 flex flex-wrap gap-1 text-[11px]">
                  <Badge tone="brand">
                    {t('config.rolloutPercent', { percent: flag.rollout.percent })}
                  </Badge>
                  {flag.rollout.zoneIds.length ? (
                    <Badge tone="info">
                      {t('config.rolloutZones', { count: flag.rollout.zoneIds.length })}
                    </Badge>
                  ) : null}
                  {flag.rollout.userIds.length ? (
                    <Badge tone="neutral">
                      {t('config.rolloutUsers', { count: flag.rollout.userIds.length })}
                    </Badge>
                  ) : null}
                </p>
              ) : null}
            ...
            <Can anyOf={[Permission.FEATURE_FLAGS_MANAGE]}>
              <Switch
                ...
                onCheckedChange={(enabled) => setPending({ flag, enabled })}
                label={flag.enabled ? t('common.enabled') : t('common.disabled')}
              />
            </Can>
          </li>
        ))}
      ...
      <ConfirmDialog
        ...
        description={
          pending
            ? `${pending.flag.key} → ${pending.enabled ? t('common.enabled') : t('common.disabled')}`
            : undefined
        }
        requireReason
        reasonMinLength={3}
        loading={mutation.isPending}
        onConfirm={(reason) => {
          if (pending) mutation.mutate({ key: pending.flag.key, enabled: pending.enabled, reason });
        }}
      />
    </div>
  );
...
function QueuesTab() {
  ...
  const query = useQuery({
    queryKey: queryKeys.config.queues,
    queryFn: configApi.queues,
    ...
    <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-3">
      {query.data.map((queue) => (
        <Card
          key={queue.queue}
          title={queue.queue}
          description={queue.paused ? t('config.queuePaused') : t('config.queueRunning')}
        >
          <dl className="grid grid-cols-3 gap-2 text-center text-xs">
            <div>
              <dt className="text-text-tertiary">{t('config.waiting')}</dt>
              <dd className="tabular text-lg font-bold">{queue.waiting}</dd>
            </div>
            <div>
              <dt className="text-text-tertiary">{t('config.active')}</dt>
              <dd className="tabular text-lg font-bold">{queue.active}</dd>
            </div>
            <div>
              <dt className="text-text-tertiary">{t('config.delayed')}</dt>
              <dd className="tabular text-lg font-bold">{queue.delayed}</dd>
            </div>
            <div>
              <dt className="text-text-tertiary">{t('config.failed')}</dt>
              <dd className="tabular text-lg font-bold text-danger">{queue.failed}</dd>
            </div>
            <div>
              <dt className="text-text-tertiary">{t('config.completed')}</dt>
              <dd className="tabular text-lg font-bold text-success">{queue.completed}</dd>
            </div>
          </dl>
        </Card>
      ))}
