...
function DispatchConsole() {
  ...
  const filters = useMemo(
    () => ({
      ...
      jobType: (jobType || undefined) as 'RIDE' | undefined,
      onlyUnassigned,
      onlyProblem,
    }),
    [zoneId, jobType, onlyUnassigned, onlyProblem],
  );
  ...
  const columns: Column<DispatchConsoleRow>[] = [
    {
      ...
      cell: (r) => (
        <div className="min-w-0">
          <Link href={`/jobs/${r.job.id}`} className="font-semibold text-primary hover:underline">
            {r.job.number}
          </Link>
          <p className="flex flex-wrap items-center gap-1 text-xs text-text-secondary">
            <StatusPill group="jobType" value={r.job.type} /> {zones.nameOf(r.job.zoneId)}
          </p>
        </div>
      ),
    },
    {
      ...
      cell: (r) => <StatusPill group="jobStatus" value={r.job.status} />,
    },
    {
      key: 'problems',
      header: t('dispatch.problems'),
      cell: (r) => (
        <div className="flex flex-wrap gap-1">
          {r.problems.length === 0 ? (
            <Badge tone="success">{t('dispatch.ok')}</Badge>
          ) : (
            r.problems.map((p) => <StatusPill key={p} group="dispatchProblem" value={p} />)
          )}
        </div>
      ),
    },
    {
      key: 'waiting',
      header: t('dispatch.waiting'),
      align: 'end',
      cell: (r) => (
        <span className={`tabular ${r.waitingSeconds > 300 ? 'font-bold text-danger' : ''}`}>
          {formatDuration(r.waitingSeconds)}
        </span>
      ),
    },
    {
      key: 'wave',
      header: t('dispatch.offers'),
      cell: (r) => (
        <span className="text-xs text-text-secondary" dir="ltr">
          W{r.wave} · {r.offersSent} {t('dispatch.sent')} / {r.offersPending}{' '}
          {t('dispatch.pending')} / {r.offersRejected} {t('dispatch.rejected')} / {r.offersExpired}{' '}
          {t('dispatch.expired')}
        </span>
      ),
    },
      ...
      cell: (r) =>
        r.partner ? (
          <button
            ...
            onClick={() => setTimelinePartner(r.partner?.id ?? null)}
          >
            <span className="block font-medium">{r.partner.fullName ?? r.partner.phone}</span>
            <span className="block text-xs text-text-secondary">
              <StatusPill group="availability" value={r.partner.availability} />{' '}
              {r.partner.heartbeatAgeSeconds !== null
                ? `♥ ${formatDuration(r.partner.heartbeatAgeSeconds)}`
                : ''}
            </span>
          </button>
        ) : (
    ...
    {
      key: 'pickup',
      header: t('jobs.pickup'),
      cell: (r) => (
        <span className="line-clamp-2 max-w-[220px] text-xs text-text-secondary">
          {r.job.stops[0]?.address.formatted ?? '—'}
        </span>
      ),
    },
    {
      ...
      cell: (r) => <DateTime value={r.job.createdAt} mode="relative" />,
    },
    {
      key: 'actions',
      ...
      cell: (r) => (
        <div className="flex justify-end gap-1">
          <Can anyOf={[Permission.DISPATCH_MANUAL_ASSIGN]}>
            <Button size="sm" variant="secondary" onClick={() => setAssignJob(r.job)}>
              {t('dispatch.assign')}
            </Button>
          </Can>
          <Can anyOf={[Permission.DISPATCH_REASSIGN]}>
            <Button size="sm" variant="outline" onClick={() => setRedispatchJob(r.job)}>
  ...
  return (
    <div>
      ...
      <FilterBar>
        ...
        <Checkbox
          checked={onlyProblem}
          onCheckedChange={setOnlyProblem}
          label={t('dispatch.onlyProblem')}
        />
        <Checkbox
          checked={onlyUnassigned}
          onCheckedChange={setOnlyUnassigned}
          label={t('dispatch.onlyUnassigned')}
        />
      </FilterBar>
      <DataTable
        columns={columns}
        ...
        emptyTitle={t('dispatch.emptyTitle')}
        emptyDescription={t('dispatch.emptyDescription')}
        rowClassName={(r) =>
          r.problems.includes('NO_PARTNER_AVAILABLE') || r.problems.includes('ETA_EXCEEDED')
            ? 'bg-danger-soft/30'
            : undefined
        }
      />
      ...
      <ConfirmDialog
        open={!!redispatchJob}
        onOpenChange={(o) => !o && setRedispatchJob(null)}
        title={t('dispatch.redispatchTitle')}
        description={
          redispatchJob
            ? t('dispatch.redispatchDescription', { job: redispatchJob.number })
            : undefined
        }
        loading={redispatch.isPending}
        confirmLabel={t('dispatch.redispatch')}
        onConfirm={() => {
          if (redispatchJob) redispatch.mutate(redispatchJob.id);
        }}
      />
    </div>
  );
