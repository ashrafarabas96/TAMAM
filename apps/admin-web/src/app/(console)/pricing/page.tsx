    ...
    <div>
      ...
      <Tabs
        items={[
          ...
          {
            value: 'cancellation',
            label: t('pricing.cancellationPolicies'),
...
function RulesTab() {
  ...
  const filters = useMemo(
    () => ({ jobType: jobType || undefined, zoneId: zoneId || undefined }),
    [jobType, zoneId],
  );
  const query = useQuery({
    queryKey: queryKeys.pricing.rules(filters),
    queryFn: () => pricingApi.rules(filters),
  });
  ...
  const columns: Column<PricingRuleRow>[] = [
    ...
    {
      key: 'jobType',
      header: t('common.jobType'),
      cell: (r) => <StatusPill group="jobType" value={r.jobType} />,
    },
    {
      key: 'scope',
      ...
      cell: (r) => (
        <span className="flex flex-wrap gap-1 text-xs">
          ...
          {r.vehicleTypeId ? (
            <Badge tone="brand">{vehicleTypes.nameOf(r.vehicleTypeId)}</Badge>
          ) : null}
    ...
    { key: 'priority', header: t('pricing.priority'), align: 'end', cell: (r) => r.priority },
    {
      ...
      cell: (r) => (
        <span className="text-xs">
          <DateTime value={r.validFrom} mode="date" />
          {r.validTo ? (
            <>
              {' '}
              → <DateTime value={r.validTo} mode="date" />
            </>
          ) : (
            ''
          )}
    ...
    {
      key: 'rule',
      header: t('pricing.ruleJson'),
      cell: (r) => (
        <details>
          <summary className="cursor-pointer text-xs text-primary">{t('common.view')}</summary>
          <JsonView value={r.rule} className="mt-1 max-h-52 max-w-md" />
        </details>
      ),
    },
    {
      ...
      align: 'end',
      cell: (r) => (
        <Can anyOf={[Permission.PRICING_MANAGE]}>
  ...
  return (
    <div>
      ...
      <DataTable
        ...
        emptyTitle={t('pricing.noRules')}
        emptyDescription={t('pricing.noRulesHint')}
      />
      <RuleDialog
        rule={editing}
...
function RuleDialog({
  ...
  const form = useForm<UpsertPricingRuleInput>({
    ...
    defaultValues: {
      ...
      priority: 0,
      isActive: true,
      rule: defaultRuleFor(JobType.RIDE) as UpsertPricingRuleInput['rule'],
      reason: '',
    },
  });
    ...
    form.reset(
      rule
        ? {
            ...
            currency: rule.currency as 'ILS',
            name: rule.name,
            priority: rule.priority,
            validFrom: rule.validFrom,
            validTo: rule.validTo,
            isActive: rule.isActive,
            rule: rule.rule as UpsertPricingRuleInput['rule'],
            reason: '',
          }
        : {
            jobType: JobType.RIDE,
  ...
  const mutation = useMutation({
    mutationFn: (input: UpsertPricingRuleInput) =>
      rule ? pricingApi.updateRule(rule.id, input) : pricingApi.createRule(input),
    onSuccess: async () => {
  ...
  return (
    <Dialog
      ...
      size="xl"
      title={rule ? t('pricing.editRule') : t('pricing.newRule')}
      description={t('pricing.ruleDialogHint')}
    ...
    >
      <form onSubmit={submit} className="space-y-6">
        <FormSection title={t('pricing.scope')}>
          <FormGrid cols={3}>
            ...
            <NativeSelectField
              ...
              label={t('common.zone')}
              options={zones.options}
              placeholder={t('pricing.allZones')}
              nullable
            />
            <NativeSelectField
              control={form.control}
              name="vehicleTypeId"
              label={t('services.vehicleTypes')}
        ...
        <button
          ...
          onClick={() =>
            form.setValue('rule', defaultRuleFor(jobType) as UpsertPricingRuleInput['rule'], {
              shouldDirty: true,
            })
          }
        >
...
function SurgeTab() {
  ...
  const query = useQuery({
    queryKey: queryKeys.pricing.surge(filters),
    queryFn: () => pricingApi.surge(filters),
  });

  const form = useForm<SurgeOverrideInput>({
    resolver: zodResolver(surgeOverrideSchema),
    defaultValues: {
      zoneId: '',
      jobType: JobType.RIDE,
      multiplier: 1.2,
  ...
  const createMutation = useMutation({
    mutationFn: (input: SurgeOverrideInput) =>
      pricingApi.createSurge({
        ...input,
        startsAt: fromDateTimeLocalValue(input.startsAt) ?? input.startsAt,
        endsAt: fromDateTimeLocalValue(input.endsAt) ?? input.endsAt,
      }),
    onSuccess: async () => {
  ...
  const columns: Column<SurgeOverrideRow>[] = [
    { key: 'zone', header: t('common.zone'), cell: (s) => zones.nameOf(s.zoneId) },
    {
      key: 'jobType',
      header: t('common.jobType'),
      cell: (s) => <StatusPill group="jobType" value={s.jobType} />,
    },
    {
      key: 'multiplier',
      header: t('pricing.multiplier'),
      align: 'end',
      cell: (s) => <span className="tabular font-bold">×{Number(s.multiplier).toFixed(2)}</span>,
    },
    {
      key: 'window',
      ...
      cell: (s) => (
        <span className="text-xs">
          <DateTime value={s.startsAt} /> → <DateTime value={s.endsAt} />
        </span>
      ),
    },
    {
      ...
      cell: (s) => <span className="line-clamp-2 max-w-[240px] text-xs">{s.reason}</span>,
    },
    {
      key: 'actions',
      header: t('common.actions'),
      ...
      cell: (s) => (
        <Can anyOf={[Permission.PRICING_MANAGE]}>
          <Button size="sm" variant="danger-soft" onClick={() => setEnding(s)}>
            {t('pricing.endSurge')}
          </Button>
        </Can>
      ),
  ...
  return (
    <div>
      ...
      <DataTable
        ...
        error={query.error}
        onRetry={() => void query.refetch()}
        emptyTitle={t('pricing.noSurge')}
        emptyDescription={t('pricing.noSurgeHint')}
      />
      <Dialog
        open={creating}
        onOpenChange={setCreating}
        title={t('pricing.newSurge')}
        ...
        <form onSubmit={submit} className="space-y-4">
          <FormGrid cols={2}>
            <NativeSelectField
              ...
              name="zoneId"
              label={t('common.zone')}
              options={zones.options.filter((z) => z.value !== '')}
            ...
            <NativeSelectField
              ...
              name="jobType"
              label={t('common.jobType')}
              options={V1_TYPES.map((j) => ({ value: j, label: j }))}
              required
            />
            <NumberField
              control={form.control}
      ...
      <ConfirmDialog
        ...
        title={t('pricing.endSurge')}
        description={
          ending
            ? `${zones.nameOf(ending.zoneId)} · ×${Number(ending.multiplier).toFixed(2)}`
            : undefined
        }
        tone="danger"
        loading={endMutation.isPending}
        confirmLabel={t('pricing.endSurge')}
        onConfirm={() => {
          if (ending) endMutation.mutate(ending.id);
        }}
      />
    </div>
  );
...
function CancellationTab() {
  ...
  const [creating, setCreating] = useState(false);
  const query = useQuery({
    queryKey: queryKeys.pricing.cancellation,
    queryFn: pricingApi.cancellationPolicies,
  });
  const columns: Column<CancellationPolicyRow>[] = [
    {
      ...
      cell: (p) =>
        p.jobType ? (
          <StatusPill group="jobType" value={p.jobType} />
        ) : (
          <Badge tone="neutral">{t('common.all')}</Badge>
        ),
    },
    {
      key: 'zone',
      header: t('common.zone'),
      cell: (p) => (p.zoneId ? zones.nameOf(p.zoneId) : t('pricing.allZones')),
    },
    {
      key: 'grace',
      header: t('pricing.gracePeriod'),
      align: 'end',
      cell: (p) => `${p.gracePeriodSeconds}s`,
    },
    {
      key: 'before',
      header: t('pricing.feeBeforeArrival'),
      align: 'end',
      cell: (p) => <MinorMoney amount={p.feeBeforeArrivalMinor} currency={p.currency} />,
    },
    {
      key: 'after',
      header: t('pricing.feeAfterArrival'),
      align: 'end',
      cell: (p) => <MinorMoney amount={p.feeAfterArrivalMinor} currency={p.currency} />,
    },
    {
      key: 'start',
      header: t('pricing.feeAfterStart'),
      align: 'end',
      cell: (p) => <MinorMoney amount={p.feeAfterStartMinor} currency={p.currency} />,
    },
    {
      key: 'partner',
      header: t('pricing.partnerFee'),
      align: 'end',
      cell: (p) => (
        <span className="text-xs">
          <MinorMoney amount={p.partnerFeeOnCancelMinor} currency={p.currency} /> ·{' '}
          {p.partnerPenaltyPoints} {t('pricing.points')}
        </span>
      ),
    },
    {
      key: 'noshow',
      header: t('pricing.customerNoShow'),
      align: 'end',
      cell: (p) => <MinorMoney amount={p.customerNoShowFeeMinor} currency={p.currency} />,
    },
    {
      key: 'active',
      header: t('common.active'),
    ...
    <div>
      <FilterBar>
        <Can anyOf={[Permission.PRICING_MANAGE]}>
          <Button size="sm" className="ms-auto" onClick={() => setCreating(true)}>
            <Plus className="h-4 w-4" aria-hidden />
            {t('pricing.newCancellationPolicy')}
          </Button>
        </Can>
      </FilterBar>
      <DataTable
        ...
        error={query.error}
        onRetry={() => void query.refetch()}
        emptyTitle={t('pricing.noCancellationPolicies')}
      />
      <CancellationDialog
        policy={editing}
        open={!!editing || creating}
...
function CancellationDialog({
  ...
  const form = useForm<UpsertCancellationPolicyInput>({
    ...
    defaultValues: {
      ...
      feeBeforeArrival: 500,
      feeAfterArrival: 1000,
      feeAfterStart: 0,
      partnerFeeOnCancel: 0,
    ...
    form.reset(
      policy
        ? {
            ...
            feeBeforeArrival: Number(policy.feeBeforeArrivalMinor),
            feeAfterArrival: Number(policy.feeAfterArrivalMinor),
            feeAfterStart: Number(policy.feeAfterStartMinor),
            partnerFeeOnCancel: Number(policy.partnerFeeOnCancelMinor),
            partnerPenaltyPoints: policy.partnerPenaltyPoints,
            customerNoShowFee: Number(policy.customerNoShowFeeMinor),
            currency: policy.currency as 'ILS',
  ...
  const mutation = useMutation({
    mutationFn: (input: UpsertCancellationPolicyInput) =>
      policy
        ? pricingApi.updateCancellationPolicy(policy.id, input)
        : pricingApi.createCancellationPolicy(input),
    onSuccess: async () => {
  ...
  return (
    <Dialog
      ...
      onOpenChange={onOpenChange}
      size="lg"
      title={policy ? t('pricing.editCancellationPolicy') : t('pricing.newCancellationPolicy')}
      locked={mutation.isPending}
      footer={
        <>
          <Button variant="ghost" onClick={() => onOpenChange(false)}>
    ...
    >
      <form onSubmit={submit} className="space-y-4">
        <FormGrid cols={3}>
          ...
          <MinorAmountField
            control={form.control}
            name="feeBeforeArrival"
            label={t('pricing.feeBeforeArrival')}
            currency={currency}
            required
          />
          <MinorAmountField
            control={form.control}
            name="feeAfterArrival"
            label={t('pricing.feeAfterArrival')}
            currency={currency}
            required
          />
          <MinorAmountField
            control={form.control}
            name="feeAfterStart"
            label={t('pricing.feeAfterStart')}
            currency={currency}
          />
          <MinorAmountField
            control={form.control}
            name="partnerFeeOnCancel"
            label={t('pricing.partnerFee')}
            currency={currency}
          />
          <NumberField
            control={form.control}
            name="partnerPenaltyPoints"
            label={t('pricing.points')}
            min={0}
            max={10}
          />
          <MinorAmountField
            control={form.control}
            name="customerNoShowFee"
            label={t('pricing.customerNoShow')}
            currency={currency}
          />
          <SwitchField control={form.control} name="isActive" label={t('common.active')} />
        </FormGrid>
