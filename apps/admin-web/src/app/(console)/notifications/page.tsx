...
export default function NotificationsPage() {
  return (
    <RequirePermission
      anyOf={[Permission.NOTIFICATION_TEMPLATES_MANAGE, Permission.NOTIFICATIONS_BROADCAST]}
    >
      <NotificationsScreen />
    </RequirePermission>
  );
        ...
        items={[
          {
            ...
            content: (
              <Can anyOf={[Permission.NOTIFICATION_TEMPLATES_MANAGE]}>
                <TemplatesTab />
              </Can>
            ),
          },
          {
            ...
            content: (
              <Can anyOf={[Permission.NOTIFICATIONS_BROADCAST]}>
                <BroadcastTab />
              </Can>
            ),
          },
...
function TemplatesTab() {
  ...
  const form = useForm<UpsertNotificationTemplateInput>({
    resolver: zodResolver(upsertNotificationTemplateSchema),
    defaultValues: {
  ...
  useEffect(() => {
    ...
    form.reset(
      editing
        ? {
            event: editing.event as UpsertNotificationTemplateInput['event'],
            channel: editing.channel as UpsertNotificationTemplateInput['channel'],
            title: { ar: editing.titleAr, en: editing.titleEn },
            body: { ar: editing.bodyAr, en: editing.bodyEn },
  ...
  const columns: Column<NotificationTemplateRow>[] = [
    ...
    },
    {
      ...
      cell: (row) => <Badge tone="brand">{enumLabel('notificationChannel', row.channel)}</Badge>,
    },
    {
      key: 'titleAr',
      header: `${t('notifications.templateTitle')} (AR)`,
      cell: (row) => (
        <span className="line-clamp-1 max-w-[200px]" dir="rtl">
          {row.titleAr}
        </span>
      ),
    },
    {
      key: 'titleEn',
      header: `${t('notifications.templateTitle')} (EN)`,
      cell: (row) => (
        <span className="line-clamp-1 max-w-[200px]" dir="ltr">
          {row.titleEn}
        </span>
      ),
    },
    ...
    },
    {
      ...
      cell: (row) => <DateTime value={row.updatedAt} mode="relative" />,
    },
    {
      key: 'actions',
  ...
  return (
    <div className="space-y-3">
      ...
      <DataTable
        ...
        emptyTitle={t('notifications.noTemplates')}
      />
      <Dialog
        ...
        size="lg"
        title={editing ? t('notifications.editTemplate') : t('notifications.newTemplate')}
        description={t('notifications.templateHint')}
        ...
        <form onSubmit={submit} className="space-y-4">
          <FormGrid cols={2}>
            <NativeSelectField
              ...
              options={Object.values(NotificationEvent).map((e) => ({ value: e, label: e }))}
              required
            />
            <NativeSelectField
              control={form.control}
              ...
              options={Object.values(NotificationChannel).map((c) => ({
                value: c,
                label: enumLabel('notificationChannel', c),
              }))}
              required
            />
          </FormGrid>
          <LocalizedTextField
            control={form.control}
            name="title"
            label={t('notifications.templateTitle')}
            required
          />
          <LocalizedTextField
            control={form.control}
            name="body"
            label={t('notifications.templateBody')}
            multiline
            required
          />
    ...
    mutationFn: (input: BroadcastNotificationInput) =>
      notificationsApi.broadcast({
        ...
        scheduledFor: input.scheduledFor
          ? (fromDateTimeLocalValue(input.scheduledFor) ?? undefined)
          : undefined,
      }),
    onSuccess: () => {
      ...
      <form className="space-y-4" onSubmit={form.handleSubmit(() => setConfirming(true))}>
        ...
        <FormGrid cols={2}>
          <TextField
            ...
            dir="ltr"
            hint={t('campaigns.actionHint.DEEP_LINK')}
          />
          <TextField
            control={form.control}
            name="scheduledFor"
