  ...
  return (
    <div className="space-y-5">
      <PageHeader
        ...
        title={
          <span className="flex items-center gap-3">
            <Avatar name={u.fullName} src={u.profileImageUrl} /> {u.fullName ?? u.phone}
          </span>
        }
        badge={
          <>
            <StatusPill group="accountStatus" value={u.accountStatus} />
            {u.roles.map((r) => (
        ...
        actions={
          <Can anyOf={[Permission.CUSTOMERS_SUSPEND]}>
            <Button variant="danger-soft" size="sm" onClick={() => setStatusOpen(true)}>
        ...
        <KeyValue
          ...
          items={[
            ...
            { label: t('common.currency'), value: u.currency },
            {
              ...
              value: u.customer
                ? `★ ${u.customer.rating.toFixed(2)} (${u.customer.ratingCount})`
                : '—',
            },
            {
              label: t('customers.completedJobs'),
              value: u.customer
                ? `${u.customer.completedJobs} / ${u.customer.cancelledJobs} ${t('customers.cancelledShort')}`
                : '—',
            },
            {
              label: t('customers.referralCode'),
              value: u.customer?.referralCode ? (
                <Badge tone="accent">{u.customer.referralCode}</Badge>
              ) : (
                '—'
              ),
            },
        ...
        items={[
          {
            ...
            content: permissions.can(Permission.JOBS_READ_ALL) ? (
              <JobsList fixed={{ customerId: u.id }} />
            ) : (
          ...
          },
          {
            ...
            content: <ReviewsTab userId={u.id} />,
          },
          {
            value: 'risk',
            label: t('nav.risk'),
            content: permissions.can(Permission.RISK_READ) ? (
              <div className="space-y-6">
                <RiskSignalsTable filters={{ userId: u.id }} showUser={false} />
                <RestrictionsTable filters={{ targetType: 'USER', targetId: u.id }} />
              </div>
            ) : (
              <p className="text-xs text-text-secondary">{t('errors.forbiddenDescription')}</p>
            ),
      ...
      <AccountStatusDialog
        ...
        subject={u.fullName ?? u.phone}
        submit={(input) => customersApi.changeStatus(u.id, input)}
        onDone={() => queryClient.invalidateQueries({ queryKey: queryKeys.customers.all })}
      />
    </div>
  );
}
...
function ReviewsTab({ userId }: { userId: string }) {
  const { t } = useI18n();
  const list = useCursorList<ReceivedReviewDto>({
    queryKey: queryKeys.customers.reviews(userId),
    fetchPage: (cursor) => customersApi.reviews(userId, { cursor, limit: 30 }),
  });
  const columns: Column<ReceivedReviewDto>[] = [
    {
      ...
      cell: (r) => (
        <span className="font-semibold text-warning-strong">
          {'★'.repeat(r.rating)}
          <span className="text-text-tertiary">{'★'.repeat(5 - r.rating)}</span>
        </span>
      ),
    },
    {
      ...
      cell: (r) => <Badge tone="neutral">{r.direction}</Badge>,
    },
    ...
    {
      key: 'comment',
      header: t('customers.comment'),
      cell: (r) => <span className="line-clamp-2 max-w-[300px] text-xs">{r.comment ?? '—'}</span>,
    },
  ...
  return (
    <DataTable
      ...
      emptyTitle={t('customers.noReviews')}
      dense
    />
  );
}
