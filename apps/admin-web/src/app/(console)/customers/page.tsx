...
function CustomersList() {
  ...
  const filters = useMemo(
    () => ({
      q: q || undefined,
      status: (status || undefined) as UserDto['accountStatus'] | undefined,
    }),
    [q, status],
  );
  const list = useCursorList<UserDto>({
    queryKey: queryKeys.customers.list(filters),
    fetchPage: (cursor) => customersApi.list({ ...filters, cursor, limit: 30 }),
  });
  const columns: Column<UserDto>[] = [
    ...
    { key: 'phone', header: t('common.phone'), cell: (u) => <span dir="ltr">{u.phone}</span> },
    {
      ...
      cell: (u) => <span dir="ltr">{u.email ?? '—'}</span>,
    },
    {
      key: 'status',
      header: t('common.status'),
      cell: (u) => <StatusPill group="accountStatus" value={u.accountStatus} />,
    },
    {
      key: 'jobs',
      header: t('customers.completedJobs'),
      align: 'end',
      cell: (u) => (
        <span className="tabular">
          {u.customer?.completedJobs ?? 0} /{' '}
          <span className="text-danger">{u.customer?.cancelledJobs ?? 0}</span>
        </span>
      ),
    },
    {
      ...
      cell: (u) => (
        <span className="tabular">
          {u.customer ? `★ ${u.customer.rating.toFixed(1)} (${u.customer.ratingCount})` : '—'}
        </span>
      ),
    },
    { key: 'lang', header: t('common.language'), cell: (u) => u.language.toUpperCase() },
    {
      ...
      cell: (u) => <DateTime value={u.createdAt} mode="date" />,
    },
  ];
  return (
    <div>
