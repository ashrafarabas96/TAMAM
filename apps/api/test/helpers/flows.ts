...
export async function runCashRide(
  ...
  const destination = { lat: RAMALLAH.lat + 0.01, lng: RAMALLAH.lng + 0.011, formatted: 'البيرة — شارع القدس', city: 'Al-Bireh' };
  ...
  const job = (
    await api
      ...
      .send({ type: 'RIDE', estimateId: estimate.estimateId, vehicleTypeId: options.vehicleTypeId, paymentMethod: 'CASH', scheduling: 'NOW', pickup, destination })
      .expect(201)
  ).body as JobDto;
  ...
  await api
    ...
    .set(partner.headers)
    .send({ status: 'ONLINE', activeRoles: [options.role ?? 'DRIVER'], activeVehicleId: options.activeVehicleId, location: sampleAt(pickup.lat + 0.001, pickup.lng) })
    .expect(200);
  ...
  const assigned = (await api.request().get(api.url(`jobs/${job.id}`)).set(partner.headers).expect(200)).body as JobDto;
  const enRoute = (await api.request().post(api.url(`jobs/${job.id}/en-route`)).set(partner.headers).send({ version: assigned.version }).expect(200)).body as JobDto;
  const arrived = (
    await api.request().post(api.url(`jobs/${job.id}/arrive`)).set(partner.headers).send({ version: enRoute.version, location: sampleAt(pickup.lat, pickup.lng) }).expect(200)
  ).body as JobDto;

  const customerView = (await api.request().get(api.url(`jobs/${job.id}`)).set(customer.headers).expect(200)).body as JobDto;
  const started = (
    await api
      .request()
      .post(api.url(`jobs/${job.id}/start`))
      .set(partner.headers)
      .send({ version: arrived.version, ...(customerView.tripPin ? { tripPin: customerView.tripPin } : {}), location: sampleAt(pickup.lat, pickup.lng) })
      .expect(200)
  ).body as JobDto;
  ...
  const payment = await waitFor(
    async () => {
      const res = await api.request().get(api.url(`jobs/${job.id}/payment`)).set(customer.headers);
  ...
  return { jobId: job.id, jobNumber: job.number, finalTotalMinor: completed.finalTotal?.amount ?? 0, paymentId: payment.id };
}
