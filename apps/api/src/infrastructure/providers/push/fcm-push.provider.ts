        ...
        const body = {
          message: {
            ...
            android: {
              ...
              ttl: `${message.ttlSeconds ?? 3600}s`,
              collapse_key: message.collapseKey,
              notification: {
                channel_id: message.priority === 'high' ? 'tamam_urgent' : 'tamam_default',
                sound: 'default',
              },
            },
            apns: {
        ...
        try {
          ...
          if (res.ok) {
            ...
            if (res.status === 404 || /UNREGISTERED|INVALID_ARGUMENT/.test(text))
              ...
              this.logger.warn({ status: res.status, text: text.slice(0, 300) }, 'FCM send failed');
          }
        } catch (err) {
          result.failed += 1;
