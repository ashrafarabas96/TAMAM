...
export class OsrmMapsProvider implements MapsProvider {
  ...
  async route(
    ...
    try {
      const data = await withRetry(() =>
        fetchJson<OsrmRoute>(url, { provider: 'osrm', timeoutMs: 6000 }),
      );
      ...
      return {
        ...
        durationSeconds: estimateEtaSeconds(d),
        polyline: null,
      };
    }
  }
    ...
    try {
      ...
      const rows = origins.map((_, i) =>
        destinations.map((_, j) => {
          ...
          return dur === null || dur === undefined
            ? null
            : { distanceMeters: Math.round(dist ?? 0), durationSeconds: Math.round(dur) };
        }),
      );
      return { rows };
    } catch (err) {
  ...
  async geocode(query: string, language: 'ar' | 'en', bias?: GeoPoint): Promise<GeocodeResult[]> {
    ...
    if (bias) {
      params.set(
        'viewbox',
        `${bias.lng - 0.3},${bias.lat + 0.3},${bias.lng + 0.3},${bias.lat - 0.3}`,
      );
    ...
    const data = await fetchJson<NominatimResult[]>(url, {
      provider: 'nominatim',
      timeoutMs: 6000,
      headers: { 'User-Agent': 'TAMAM/1.0 (support@tamam.app)' },
    });
    return data.map((r) => ({
  ...
  async reverseGeocode(point: GeoPoint, language: 'ar' | 'en'): Promise<GeocodeResult | null> {
    ...
    try {
      const r = await fetchJson<NominatimResult>(url, {
        provider: 'nominatim',
        timeoutMs: 6000,
        headers: { 'User-Agent': 'TAMAM/1.0 (support@tamam.app)' },
      });
  ...
  async placeSearch(
    ...
    return results.map((r) => {
      ...
      return {
        placeId: r.placeId ?? r.formatted,
        primaryText: (primary ?? r.formatted).trim(),
