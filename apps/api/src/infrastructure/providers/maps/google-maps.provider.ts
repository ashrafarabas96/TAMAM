...
export class GoogleMapsProvider implements MapsProvider {
  ...
  async route(
    ...
    const data = await fetchJson<GDirections>(
  ...
  async distanceMatrix(
    ...
    const data = await fetchJson<GMatrix>(
      ...
      rows: data.rows.map((row) =>
        row.elements.map((e) =>
          e.status === 'OK' && e.distance && e.duration
            ? { distanceMeters: e.distance.value, durationSeconds: e.duration.value }
            : null,
        ),
      ),
    };
  ...
  async geocode(query: string, language: 'ar' | 'en', bias?: GeoPoint): Promise<GeocodeResult[]> {
    ...
    if (bias)
      params.set(
        'bounds',
        `${bias.lat - 0.3},${bias.lng - 0.3}|${bias.lat + 0.3},${bias.lng + 0.3}`,
      );
    const data = await fetchJson<GGeocode>(
  ...
  async placeSearch(
    ...
    const data = await fetchJson<GAutocomplete>(
