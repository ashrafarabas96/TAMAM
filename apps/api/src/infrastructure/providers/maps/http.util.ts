...
export async function fetchJson<T>(
  ...
  try {
    ...
    if (!res.ok)
      throw AppException.external(init.provider, `${init.provider} responded ${res.status}`);
    return (await res.json()) as T;
  } catch (err) {
    if (err instanceof AppException) throw err;
    throw AppException.external(
      init.provider,
...
export async function withRetry<T>(
  ...
  for (let i = 0; i <= attempts; i += 1) {
    try {
      ...
      lastErr = err;
      if (i < attempts)
        await new Promise((r) => setTimeout(r, baseDelayMs * (i + 1) + Math.random() * 100));
    }
  }
  throw lastErr;
}
