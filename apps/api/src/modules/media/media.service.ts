...
const ALLOWED: Record<MediaKind, { mimes: string[]; maxBytes: number; extensions: string[] }> = {
  IMAGE: { mimes: ['image/jpeg', 'image/png', 'image/webp', 'image/heic'], maxBytes: 15 * 1024 * 1024, extensions: ['jpg', 'jpeg', 'png', 'webp', 'heic'] },
  VIDEO: { mimes: ['video/mp4', 'video/quicktime', 'video/webm'], maxBytes: 120 * 1024 * 1024, extensions: ['mp4', 'mov', 'webm'] },
  AUDIO: { mimes: ['audio/mpeg', 'audio/mp4', 'audio/aac', 'audio/webm', 'audio/ogg', 'audio/x-m4a'], maxBytes: 20 * 1024 * 1024, extensions: ['mp3', 'm4a', 'aac', 'webm', 'ogg'] },
  DOCUMENT: { mimes: ['application/pdf', 'image/jpeg', 'image/png'], maxBytes: 20 * 1024 * 1024, extensions: ['pdf', 'jpg', 'jpeg', 'png'] },
};

/** Purposes whose objects may live in the public bucket (everything else is private + signed). */
const PUBLIC_PURPOSES: MediaPurpose[] = [MediaPurpose.BANNER_CREATIVE, MediaPurpose.SERVICE_ICON, MediaPurpose.PROFILE];
...
export class MediaService {
  ...
  async createUploadIntent(user: RequestUser, input: MediaUploadIntentInput) {
    ...
    if (!rule.mimes.includes(input.mimeType.toLowerCase())) throw AppException.badRequest(ErrorCode.UPLOAD_INVALID, `MIME type ${input.mimeType} is not allowed for ${input.kind}`);
    if (input.sizeBytes > rule.maxBytes) throw AppException.badRequest(ErrorCode.UPLOAD_TOO_LARGE, `Max size for ${input.kind} is ${Math.round(rule.maxBytes / 1024 / 1024)} MB`);
    if (input.originalFilename) {
      const ext = input.originalFilename.split('.').pop()?.toLowerCase() ?? '';
      if (!rule.extensions.includes(ext)) throw AppException.badRequest(ErrorCode.UPLOAD_INVALID, `Extension .${ext} is not allowed`);
    }
    const staffRule = STAFF_PURPOSES.find((s) => s.purpose === input.purpose);
    if (staffRule && !user.isSuperAdmin && !user.permissions.includes(staffRule.permission)) throw AppException.forbidden();
    ...
    const media = await this.prisma.mediaAsset.create({
      data: { uploaderId: user.id, kind: input.kind, purpose: input.purpose, bucket, objectKey, mimeType: input.mimeType.toLowerCase(), sizeBytes: BigInt(input.sizeBytes), originalFilename: input.originalFilename?.slice(0, 200) ?? null, isPublic, expiresAt: new Date(Date.now() + 3600 * 1000) },
    });
    const intent = await this.storage.createUploadIntent(bucket, objectKey, media.mimeType, input.sizeBytes);
  ...
  async confirmUpload(user: RequestUser, mediaId: string) {
    ...
    if (!head.exists) throw AppException.badRequest(ErrorCode.UPLOAD_INVALID, 'Object was not uploaded');
    if (head.sizeBytes > Number(media.sizeBytes) * 1.05 + 1024) throw AppException.badRequest(ErrorCode.UPLOAD_TOO_LARGE, 'Uploaded object is larger than declared');
    ...
    if (media.kind === 'IMAGE' || media.mimeType === 'application/pdf') {
      ...
      if (expected && sniffed && sniffed.mime !== expected) {
        ...
        await this.prisma.mediaAsset.update({ where: { id: media.id }, data: { status: 'REJECTED' } });
        throw AppException.badRequest(ErrorCode.UPLOAD_INVALID, 'File content does not match its declared type');
      }
    }

    const updated = await this.prisma.mediaAsset.update({ where: { id: media.id }, data: { status: media.kind === 'IMAGE' ? 'PROCESSING' : 'READY', sizeBytes: BigInt(head.sizeBytes), expiresAt: null } });
    if (media.kind === 'IMAGE') await this.queue.add(MEDIA_JOBS.PROCESS_IMAGE, { mediaId: media.id }, { jobId: `img-${media.id}` });
  ...
  async processImage(mediaId: string): Promise<void> {
    ...
    await this.storage.putObject(media.bucket, mediumKey, await base.clone().resize({ width: 1280, height: 1280, fit: 'inside', withoutEnlargement: true }).webp({ quality: 82 }).toBuffer(), 'image/webp');
    await this.storage.putObject(media.bucket, thumbKey, await base.clone().resize({ width: 320, height: 320, fit: 'cover' }).webp({ quality: 75 }).toBuffer(), 'image/webp');
    await this.prisma.mediaAsset.update({ where: { id: mediaId }, data: { status: 'READY', width: meta.width ?? null, height: meta.height ?? null, exifStripped: true, mediumKey, thumbnailKey: thumbKey } });
  }
  ...
  async scan(mediaId: string): Promise<void> {
    await this.prisma.mediaAsset.updateMany({ where: { id: mediaId }, data: { scanStatus: 'CLEAN' } });
  }
  ...
  async resolveSigned(objectKey: string, user: RequestUser): Promise<string> {
    const media = await this.prisma.mediaAsset.findFirst({ where: { OR: [{ objectKey }, { mediumKey: objectKey }, { thumbnailKey: objectKey }] } });
    if (!media) throw AppException.notFound('Media');
    const allowed = media.isPublic || media.uploaderId === user.id || user.isSuperAdmin || (await this.canStaffView(user, media.purpose));
  ...
  async assertOwnedReady(userId: string, mediaIds: string[], purposes: MediaPurpose[]): Promise<void> {
    if (!mediaIds.length) return;
    const rows = await this.prisma.mediaAsset.findMany({ where: { id: { in: mediaIds } }, select: { id: true, uploaderId: true, status: true, purpose: true } });
    if (rows.length !== mediaIds.length) throw AppException.validation([{ field: 'mediaIds', message: 'unknown media id' }]);
    for (const r of rows) {
      if (r.uploaderId !== userId) throw AppException.forbidden('Media does not belong to you');
      if (!purposes.includes(r.purpose)) throw AppException.validation([{ field: 'mediaIds', message: `media ${r.id} has purpose ${r.purpose}` }]);
      if (r.status !== 'READY' && r.status !== 'PROCESSING' && r.status !== 'UPLOADED') throw AppException.validation([{ field: 'mediaIds', message: `media ${r.id} is not uploaded` }]);
    }
  }

  toDto(m: { id: string; kind: MediaKind; purpose: MediaPurpose; status: string; bucket: string; objectKey: string; isPublic: boolean; mediumKey: string | null; thumbnailKey: string | null; mimeType: string; sizeBytes: bigint; width: number | null; height: number | null; createdAt: Date }) {
    return { id: m.id, kind: m.kind, purpose: m.purpose, status: m.status, url: this.urls.urlFor(m, 'original'), mediumUrl: this.urls.urlFor(m, 'medium'), thumbnailUrl: this.urls.urlFor(m, 'thumbnail'), mimeType: m.mimeType, sizeBytes: Number(m.sizeBytes), width: m.width, height: m.height, createdAt: m.createdAt.toISOString() };
  }
}
