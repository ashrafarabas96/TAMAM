...
describe('computeRideFare', () => {
  it('meters distance and time in integer minor units', () => {
    const r = computeRideFare(rideRule, {
      distanceMeters: 5000,
      durationSeconds: 600,
      surgeMultiplier: 1,
    });
...
describe('computeHomeServiceFare', () => {
  ...
  it('charges only the inspection fee before a quote', () => {
    const r = computeHomeServiceFare(rule, {
      pricingMethod: 'INSPECTION_QUOTE',
  ...
  it('uses the approved quote and waives the inspection fee', () => {
    const r = computeHomeServiceFare(rule, {
      ...
      inspectionFeeMinor: null,
      urgency: JobUrgency.STANDARD,
      quote: { laborMinor: 10000n, partsMinor: 4000n, feesMinor: 0n, discountMinor: 1000n },
      quoteApproved: true,
    });
...
describe('applyPromoAndTax', () => {
  it('caps promo at subtotal and taxes the discounted amount', () => {
    const base = computeRideFare(rideRule, {
      distanceMeters: 5000,
      durationSeconds: 600,
      surgeMultiplier: 1,
    });
