...
import {
  ...
  homeServicePricingRuleSchema,
  ridePricingRuleSchema,
} from '@tamam/validation';
...
import {
  ...
  applyPromoAndTax,
  computeDeliveryFare,
  computeHomeServiceFare,
  computeRideFare,
  taxPercentOf,
  toBreakdown,
} from './domain/fare-calculator';
...
export class PricingService {
  ...
  async resolveRule(
    ...
    categoryId: string | null,
    at = new Date(),
  ): Promise<{ id: string | null; rule: AnyRule; currency: string }> {
    ...
    if (!rows.length)
      throw AppException.badRequest(
        ...
        'No pricing configured for this service in your area',
      );
    const score = (r: (typeof rows)[number]) =>
      (r.zoneId ? 4 : 0) + (r.vehicleTypeId ? 2 : 0) + (r.categoryId ? 2 : 0);
    rows.sort(
      (a, b) =>
        score(b) - score(a) ||
    ...
    const best = rows[0];
    if (!best)
      throw AppException.badRequest(ErrorCode.SERVICE_UNAVAILABLE_IN_ZONE, 'No pricing configured');
  ...
  parseRule(jobType: JobType, raw: unknown): AnyRule {
    ...
    if (!parsed.success)
      throw AppException.internal(`Stored pricing rule for ${jobType} is malformed`);
    return parsed.data;
  }

  async surgeMultiplier(
    zoneId: string,
    jobType: JobType,
    baseFromRule = 1,
    at = new Date(),
    ...
    const [override, cap] = await Promise.all([
      this.prisma.surgeOverride.findFirst({
        where: { zoneId, jobType, startsAt: { lte: at }, endsAt: { gt: at } },
  ...
  async estimateRide(
    ...
    await this.zones.assertServiceAvailable(
      zone.id,
      { serviceTypeId: await this.serviceTypeId(JobType.RIDE) },
    ...
    for (const vt of vehicleTypes) {
      ...
      const result = computeRideFare(rule, {
    ...
    if (!options.length)
      throw AppException.badRequest(
        ErrorCode.SERVICE_UNAVAILABLE_IN_ZONE,
        'Rides are not priced in your area yet',
      );
    return this.store(
      user,
      {
  ...
  async estimateDelivery(
    ...
    await this.zones.assertServiceAvailable(
      zone.id,
      { serviceTypeId: await this.serviceTypeId(JobType.DELIVERY) },
      input.scheduledFor ? new Date(input.scheduledFor) : new Date(),
    );
    ...
    if (
      ...
      input.approximateWeightKg > pkg.maxWeightKg.toNumber()
    )
      throw AppException.validation([
        {
          field: 'approximateWeightKg',
          message: `max ${pkg.maxWeightKg} kg for this package type`,
        },
      ]);
    ...
    if (pkg.requiresVehicleTypeIds.length)
      vehicleTypes = vehicleTypes.filter((v) => pkg.requiresVehicleTypeIds.includes(v.id));
    const options: EstimateOption[] = [];
    for (const vt of vehicleTypes) {
      let resolved: { id: string | null; rule: AnyRule };
      try {
      ...
      const result = computeDeliveryFare(rule, {
        distanceMeters: route.distanceMeters,
        size: input.approximateSize,
        weightKg: input.approximateWeightKg ?? null,
    ...
    if (!options.length)
      throw AppException.badRequest(
        ErrorCode.SERVICE_UNAVAILABLE_IN_ZONE,
        'Delivery is not priced in your area yet',
      );
    return this.store(
      user,
      {
  ...
  async estimateService(
    ...
    if (category.serviceType.code !== JobType.HOME_SERVICE)
      throw AppException.validation([
        { field: 'categoryId', message: 'not a home-service category' },
      ]);
    await this.zones.assertServiceAvailable(
      zone.id,
      { serviceTypeId: category.serviceTypeId, categoryId: category.id },
      input.scheduledFor ? new Date(input.scheduledFor) : new Date(),
    );
    if (!category.urgencyLevels.includes(input.urgency))
      throw AppException.validation([
        { field: 'urgency', message: 'urgency level not offered for this service' },
      ]);
    await this.assertUrgencyAllowed(input.urgency, user.id, zone.id);
    if (input.scheduledFor && !category.allowsScheduled)
      throw AppException.validation([
        { field: 'scheduledFor', message: 'this service cannot be scheduled' },
      ]);
    if (!input.scheduledFor && !category.allowsInstant)
      throw AppException.validation([
        { field: 'scheduledFor', message: 'this service must be scheduled' },
      ]);
    const sub = input.subcategoryId
      ? category.subcategories.find((s) => s.id === input.subcategoryId && s.isActive)
      : undefined;
    if (input.subcategoryId && !sub)
      throw AppException.notFound('Subcategory', input.subcategoryId);
    const chosenOptions = (sub?.options ?? []).filter(
      (o) => input.optionIds.includes(o.id) && o.isActive,
    );
    if (chosenOptions.length !== input.optionIds.length)
      throw AppException.validation([
        { field: 'optionIds', message: 'unknown or inactive option' },
      ]);
    ...
    const eta = await this.nearestPartnerEta(
      input.location,
      null,
    ...
    return this.store(
      user,
      {
        ...
        destination: null,
        route: { distanceMeters: 0, durationSeconds: 0, polyline: null },
        options: [option],
        urgency: input.urgency,
        scheduledFor: input.scheduledFor ?? null,
  ...
  async getEstimate(estimateId: string, userId: string): Promise<StoredEstimate> {
    ...
    if (new Date(est.expiresAt) < new Date())
      throw AppException.badRequest(
        ErrorCode.VALIDATION_FAILED,
        'Estimate expired — please refresh the price',
      );
    return est;
  }
  ...
  async createSnapshot(
    ...
    if (!option)
      throw AppException.validation([
        { field: 'vehicleTypeId', message: 'option is not part of this estimate' },
      ]);
    ...
    const base: FareResult = {
      totalMinor: BigInt(option.totalMinor),
      subtotalMinor: BigInt(option.subtotalMinor),
      lines: option.lines.map((l) => ({ code: l.code, amountMinor: BigInt(l.amountMinor) })),
    ...
    const snapshot = await tx.pricingSnapshot.create({
      data: {
        ...
        inputs: {
          ...
          urgency: est.urgency,
          optionIds: est.optionIds,
          taxPercent: option.taxPercent,
          promoDiscountMinor: Number(promoDiscountMinor),
          scheduledFor: est.scheduledFor,
    ...
    return {
      ...
      totalMinor: withPromo.totalMinor,
      subtotalMinor: withPromo.subtotalMinor,
      breakdown,
      option,
    };
  }
  ...
  async finalizeFare(
    ...
    const snap = await client.pricingSnapshot.findUniqueOrThrow({
      where: { id: job.pricingSnapshotId },
    });
    const inputs = snap.inputs as {
    ...
    if (job.type === JobType.RIDE) {
      result = computeRideFare(rule as RidePricingRule, {
        distanceMeters: actuals.distanceMeters ?? job.distanceMeters ?? 0,
        durationSeconds: actuals.durationSeconds ?? job.durationSeconds ?? 0,
        waitingSeconds: actuals.waitingSeconds ?? 0,
        surgeMultiplier: snap.surgeMultiplier.toNumber(),
      });
      ...
      result = computeDeliveryFare(rule as DeliveryPricingRule, {
      ...
      const category = job.categoryId
        ? await client.serviceCategory.findUnique({ where: { id: job.categoryId } })
        : null;
    ...
    return {
      ...
      subtotalMinor: final.subtotalMinor,
      breakdown: toBreakdown(final, job.currency),
  ...
  async cancellationFee(
    job: {
      ...
      partnerArrivedAt: Date | null;
      startedAt: Date | null;
      currency: string;
    },
    cancelledBy: JobActorType,
    at = new Date(),
  ): Promise<{
    customerFeeMinor: bigint;
    partnerCompensationMinor: bigint;
    partnerPenaltyPoints: number;
    policyId: string | null;
  }> {
    if (cancelledBy === JobActorType.ADMIN || cancelledBy === JobActorType.SYSTEM)
      return {
        customerFeeMinor: 0n,
        partnerCompensationMinor: 0n,
        partnerPenaltyPoints: 0,
        policyId: null,
      };
    const policies = await this.prisma.cancellationPolicy.findMany({
      where: {
        isActive: true,
        AND: [
          { OR: [{ jobType: job.type }, { jobType: null }] },
          { OR: [{ zoneId: job.zoneId }, { zoneId: null }] },
        ],
      },
    });
    policies.sort(
      (a, b) =>
        (b.jobType ? 2 : 0) + (b.zoneId ? 1 : 0) - ((a.jobType ? 2 : 0) + (a.zoneId ? 1 : 0)),
    );
    const p = policies[0];
    const [grace, feeBefore, feeAfter, penalty] = p
      ? [
          p.gracePeriodSeconds,
          p.feeBeforeArrivalMinor,
          p.feeAfterArrivalMinor,
          p.partnerPenaltyPoints,
        ]
      : [
          await this.config.getNumber(CONFIG_KEYS.CANCELLATION_GRACE_S),
          BigInt(await this.config.getNumber(CONFIG_KEYS.CANCELLATION_FEE_MINOR)),
          BigInt(await this.config.getNumber(CONFIG_KEYS.CANCELLATION_FEE_AFTER_ARRIVAL_MINOR)),
          await this.config.getNumber(CONFIG_KEYS.CANCELLATION_PARTNER_PENALTY_POINTS),
        ];
    ...
    if (job.startedAt && p && p.feeAfterStartMinor > 0n)
      return {
        customerFeeMinor: p.feeAfterStartMinor,
        partnerCompensationMinor: p.partnerFeeOnCancelMinor,
        partnerPenaltyPoints: 0,
        policyId: p.id,
      };
    if (job.partnerArrivedAt)
      return {
        customerFeeMinor: feeAfter,
        partnerCompensationMinor: p?.partnerFeeOnCancelMinor ?? 0n,
        partnerPenaltyPoints: 0,
        policyId: p?.id ?? null,
      };
    if (sinceAssign <= grace)
      return {
        customerFeeMinor: 0n,
        partnerCompensationMinor: 0n,
        partnerPenaltyPoints: 0,
  ...
  async estimatedPartnerEarnings(
    totalMinor: bigint,
    snapshot: { commissionPercent: Prisma.Decimal; commissionFixedMinor: bigint } | null,
    promoDiscountMinor = 0n,
  ): Promise<bigint> {
    const percent = snapshot
      ? snapshot.commissionPercent.toNumber()
      : await this.config.getNumber(CONFIG_KEYS.COMMISSION_DEFAULT_PERCENT);
  ...
  async listRules(filter: { jobType?: JobType; zoneId?: string }) {
    return this.prisma.pricingRule.findMany({
      where: { jobType: filter.jobType, zoneId: filter.zoneId },
      include: {
        zone: { select: { nameAr: true, nameEn: true } },
  ...
  async upsertRule(
    ...
    return this.prisma.$transaction(async (tx) => {
      ...
      await this.audit.record(
        {
          actorId,
          action: id ? 'pricing.rule.update' : 'pricing.rule.create',
          entity: 'pricing_rule',
          entityId: row.id,
          oldValue: before ? { rule: before.rule, isActive: before.isActive } : null,
          newValue: { rule: input.rule, isActive: input.isActive },
          reason: input.reason,
          requestId,
        },
        tx,
  ...
  async listSurge(zoneId?: string) {
    return this.prisma.surgeOverride.findMany({
      where: { zoneId, endsAt: { gt: new Date() } },
  ...
  async createSurge(input: SurgeOverrideInput, actorId: string, requestId: string | null) {
    ...
    if (input.multiplier > cap)
      throw AppException.badRequest(ErrorCode.CONFIG_OUT_OF_RANGE, `Surge cannot exceed ${cap}`);
    if (new Date(input.endsAt) <= new Date(input.startsAt))
      throw AppException.validation([{ field: 'endsAt', message: 'must be after startsAt' }]);
    return this.prisma.$transaction(async (tx) => {
      const row = await tx.surgeOverride.create({
        data: {
          zoneId: input.zoneId,
          jobType: input.jobType,
      ...
      await this.audit.record(
        {
          actorId,
          action: 'pricing.surge.create',
          entity: 'surge_override',
          entityId: row.id,
          newValue: input,
          reason: input.reason,
          requestId,
  ...
  async endSurge(id: string, actorId: string, requestId: string | null) {
    return this.prisma.$transaction(async (tx) => {
      ...
      await this.audit.record(
        { actorId, action: 'pricing.surge.end', entity: 'surge_override', entityId: id, requestId },
        tx,
      );
      return row;
    });
  ...
  async listCancellationPolicies() {
    return this.prisma.cancellationPolicy.findMany({
      orderBy: [{ jobType: 'asc' }, { zoneId: 'asc' }],
    });
  }
  ...
  async upsertCancellationPolicy(
    ...
    const data = {
      ...
      feeBeforeArrivalMinor: BigInt(input.feeBeforeArrival),
      feeAfterArrivalMinor: BigInt(input.feeAfterArrival),
      feeAfterStartMinor: BigInt(input.feeAfterStart),
      partnerFeeOnCancelMinor: BigInt(input.partnerFeeOnCancel),
      partnerPenaltyPoints: input.partnerPenaltyPoints,
      customerNoShowFeeMinor: BigInt(input.customerNoShowFee),
      isActive: input.isActive,
    };
    return this.prisma.$transaction(async (tx) => {
      const before = id ? await tx.cancellationPolicy.findUnique({ where: { id } }) : null;
      const row = id
        ? await tx.cancellationPolicy.update({ where: { id }, data })
        : await tx.cancellationPolicy.create({ data });
      await this.audit.record(
        {
          actorId,
          action: id ? 'pricing.cancellation.update' : 'pricing.cancellation.create',
  ...
  private async assertUrgencyAllowed(
    urgency: JobUrgency,
    userId: string,
    zoneId: string,
  ): Promise<void> {
    if (urgency !== JobUrgency.STANDARD)
      await this.config.assertEnabled('urgent_services', { userId, zoneId });
  }

  /** Straight-line + average speed ETA of the nearest eligible online partner (spec §26 — routing refinement happens on assignment). */
  private async nearestPartnerEta(
    point: GeoPoint,
    vehicleTypeId: string | null,
    jobType: JobType,
    categoryId?: string,
    ...
    const role =
      jobType === JobType.RIDE ? 'DRIVER' : jobType === JobType.DELIVERY ? 'COURIER' : 'TECHNICIAN';
  ...
  private toOption(
    ...
    return {
      ...
      lines: result.lines.map((l) => ({ code: l.code, amountMinor: l.amountMinor.toString() })),
      etaToPickupSeconds: eta,
    };
  }
  ...
  private async store(
    ...
    p: {
      jobType: JobType;
      zone: { id: string; currency: string; timezone: string };
      pickup: GeoPoint & { formatted: string };
      destination: (GeoPoint & { formatted: string }) | null;
    ...
    const est: StoredEstimate = {
      ...
      timezone: p.zone.timezone,
      categoryId: p.categoryId,
      subcategoryId: p.subcategoryId,
      optionIds: p.optionIds,
      packageCategoryId: p.packageCategoryId,
      size: p.size,
      weightKg: p.weightKg,
      urgency: p.urgency,
      scheduledFor: p.scheduledFor,
      pickup: { lat: p.pickup.lat, lng: p.pickup.lng, formatted: p.pickup.formatted },
      destination: p.destination
        ? { lat: p.destination.lat, lng: p.destination.lng, formatted: p.destination.formatted }
        : null,
      distanceMeters: p.route.distanceMeters,
      durationSeconds: p.route.durationSeconds,
      routePolyline: p.route.polyline,
      options: p.options,
      subtotalMinor: first ? Number(first.subtotalMinor) : 0,
      createdAt: now.toISOString(),
      expiresAt: addSeconds(now, ttl).toISOString(),
    };
    ...
    return {
      ...
      options: p.options.map((o): FareOptionDto => ({
        ...
        breakdown: toBreakdown(
          {
            totalMinor: BigInt(o.totalMinor),
            subtotalMinor: BigInt(o.subtotalMinor),
            lines: o.lines.map((l) => ({ code: l.code, amountMinor: BigInt(l.amountMinor) })),
