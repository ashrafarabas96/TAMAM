...
export function evaluatePromo(promo: PromoRule, ctx: PromoContext): PromoEvaluation {
  ...
  if (promo.endsAt && promo.endsAt.getTime() <= ctx.now.getTime()) return reject(ErrorCode.PROMO_EXPIRED);
  if (promo.currency !== ctx.currency) return reject(ErrorCode.PROMO_NOT_ELIGIBLE);

  if (promo.usageLimit !== null && promo.usageCount >= promo.usageLimit) return reject(ErrorCode.PROMO_USAGE_EXCEEDED);
  if (promo.perUserLimit > 0 && ctx.userRedemptions >= promo.perUserLimit) return reject(ErrorCode.PROMO_USAGE_EXCEEDED);

  if (promo.firstOrderOnly && !ctx.isFirstOrder) return reject(ErrorCode.PROMO_NOT_ELIGIBLE);
  if (promo.jobTypes.length && !promo.jobTypes.includes(ctx.jobType)) return reject(ErrorCode.PROMO_NOT_ELIGIBLE);
  if (promo.categoryIds.length && (!ctx.categoryId || !promo.categoryIds.includes(ctx.categoryId))) return reject(ErrorCode.PROMO_NOT_ELIGIBLE);
  if (promo.zoneIds.length && !promo.zoneIds.includes(ctx.zoneId)) return reject(ErrorCode.PROMO_NOT_ELIGIBLE);
  if (promo.userIds.length && !promo.userIds.includes(ctx.userId)) return reject(ErrorCode.PROMO_NOT_ELIGIBLE);
  if (promo.paymentMethods.length && !promo.paymentMethods.includes(ctx.paymentMethod)) return reject(ErrorCode.PROMO_NOT_ELIGIBLE);

  if (ctx.subtotalMinor < promo.minOrderMinor) return reject(ErrorCode.PROMO_MIN_ORDER_NOT_MET);

  let discountMinor =
    promo.type === PromoType.PERCENTAGE ? percentOf(ctx.subtotalMinor, promo.value) : BigInt(Math.max(0, Math.round(promo.value)));
  if (promo.maxDiscountMinor !== null && discountMinor > promo.maxDiscountMinor) discountMinor = promo.maxDiscountMinor;
