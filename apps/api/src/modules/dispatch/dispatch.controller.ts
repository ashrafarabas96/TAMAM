...
import {
  ...
  type NearbyPartnersQueryInput,
  type RespondToOfferInput,
  manualAssignSchema,
  nearbyPartnersQuerySchema,
...
export class DispatchController {
  ...
  @Get('partners/me/offers')
  @RequireRole('PARTNER')
  offers(@CurrentUser() user: RequestUser) {
    return this.dispatch.listOffers(user);
  }

  @Post('partners/me/offers/respond')
  @HttpCode(200)
  @RequireRole('PARTNER')
  async respond(
    @CurrentUser() user: RequestUser,
    @ZodBody(respondToOfferSchema) input: RespondToOfferInput,
  ) {
  ...
  async release(
    ...
    @ZodBody(releaseSchema) body: { reason: string },
  ) {
    await this.dispatch.releaseByPartner(id, user, body.reason);
    return { ok: true };
  }

  @Post('jobs/:id/retry-dispatch')
  @HttpCode(200)
  ...
  /* --------------------------------------------------------- dispatcher */
  @Get('admin/dispatch/nearby-partners')
  @RequirePermission(Permission.DISPATCH_MANUAL_ASSIGN)
  nearby(@ZodQuery(nearbyPartnersQuerySchema) q: NearbyPartnersQueryInput) {
    return this.dispatch.nearbyPartners(q);
  }

  @Get('admin/jobs/:id/assignments')
  @RequirePermission(Permission.JOBS_READ_ALL)
  assignments(@Param('id', UuidPipe) id: string) {
    return this.dispatch.assignmentsForJob(id);
  }
  ...
  async assign(
    ...
    @Param('id', UuidPipe) id: string,
    @ZodBody(manualAssignSchema) input: ManualAssignInput,
    @RequestId() rid: string,
  ) {
    const job = await this.dispatch.manualAssign(
      id,
      input.partnerId,
      actor,
      input.reason,
  ...
  @Post('admin/jobs/:id/redispatch')
  @HttpCode(200)
  @RequirePermission(Permission.DISPATCH_REASSIGN)
