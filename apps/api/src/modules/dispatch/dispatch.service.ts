...
import {
  MAPS_PROVIDER,
  type MapsProvider,
} from '../../infrastructure/providers/maps/maps.provider';
...
export class DispatchService {
  constructor(
    ...
    @Inject(forwardRef(() => PartnerAvailabilityService))
  ...
  async runWave(jobId: string, wave: number): Promise<void> {
    ...
    if (job.status === JobStatus.REQUESTED || job.status === JobStatus.NO_PARTNER_AVAILABLE) {
      ...
      job = await this.jobs.transition(
        jobId,
        JobStatus.SEARCHING,
        { type: JobActorType.SYSTEM, id: null },
        {
          patch: {
            dispatchStartedAt: new Date(),
            dispatchDeadlineAt: addSeconds(new Date(), timeout),
            dispatchWave: 0,
          },
        },
      );
      await this.queue.add(
        DISPATCH_JOBS.DISPATCH_TIMEOUT,
        { jobId },
        { delay: timeout * 1000, jobId: `timeout-${jobId}-${job.version}` },
      );
    }
    ...
    const [maxWaves, offerTtl] = await Promise.all([
      this.config.getNumber(CONFIG_KEYS.DISPATCH_MAX_WAVES),
      this.config.getNumber(CONFIG_KEYS.DISPATCH_OFFER_TTL_S),
    ]);
    if (wave > maxWaves) {
      await this.onTimeout(jobId);
      return;
    ...
    const earnings = await this.pricing.estimatedPartnerEarnings(
      job.estimatedTotalMinor ?? 0n,
      job.pricingSnapshot
        ? {
            commissionPercent: job.pricingSnapshot.commissionPercent,
            commissionFixedMinor: job.pricingSnapshot.commissionFixedMinor,
          }
        : null,
      job.promoDiscountMinor,
    );
    const created = await this.prisma.$transaction(async (tx) => {
      const rows = [];
      for (const c of chosen) {
        try {
          const a = await tx.jobAssignment.create({
            data: {
              jobId,
              ...
              estimatedEarningsMinor: earnings,
              expiresAt,
            },
          });
          rows.push(a);
    ...
    for (const a of created) {
      const offer: JobOfferDto = {
        assignmentId: a.id,
        job: { ...jobDto, customer: undefined },
        wave,
        expiresAt: expiresAt.toISOString(),
        distanceToPickupMeters: a.distanceMeters,
        etaToPickupSeconds: a.etaSeconds,
        estimatedEarnings: {
          amount: Number(earnings),
          currency: job.currency as JobOfferDto['estimatedEarnings']['currency'],
        },
      };
      this.events.emit(DispatchEvents.OFFER, { partnerId: a.partnerId, offer });
      await this.notifications.notify({
        userId: a.partnerId,
        event: NotificationEvent.JOB_OFFER,
        jobId,
        priority: 'high',
        collapseKey: `offer:${jobId}`,
        vars: {
          serviceName: this.serviceName(job),
          distanceKm: (a.distanceMeters / 1000).toFixed(1),
          earnings: formatMajor(earnings, job.currency),
        },
        data: { assignmentId: a.id, expiresAt: expiresAt.toISOString(), type: 'JOB_OFFER' },
      });
    }
    await this.queue.add(
      DISPATCH_JOBS.OFFER_EXPIRED,
      { jobId, wave },
      { delay: offerTtl * 1000 + 500, jobId: `expire-${jobId}-${wave}-${Date.now()}` },
    );
    this.logger.info({ jobId, wave, offered: created.length, radius }, 'dispatch wave sent');
  }

  private async scheduleNextWave(
    jobId: string,
    wave: number,
    maxWaves: number,
    delaySeconds: number,
    ...
    await this.queue.add(
      DISPATCH_JOBS.WAVE,
      { jobId, wave: wave + 1 },
      { delay: delaySeconds * 1000, jobId: `wave-${jobId}-${wave + 1}-${Date.now()}` },
    );
  }

  /** Worker: expire a wave's open offers, then continue or give up. */
  async onOffersExpired(jobId: string, wave: number): Promise<void> {
    const expired = await this.prisma.jobAssignment.findMany({
      where: { jobId, wave, status: AssignmentStatus.OFFERED },
    });
    if (expired.length) {
      await this.prisma.jobAssignment.updateMany({
        ...
        data: {
          ...
          releasedAt: new Date(),
          releaseReason: 'offer_ttl',
        },
      });
      for (const e of expired)
        this.events.emit(DispatchEvents.OFFER_EXPIRED, {
    ...
    const job = await this.prisma.job.findUnique({
      where: { id: jobId },
      select: { status: true, dispatchDeadlineAt: true },
    });
  ...
  async onTimeout(jobId: string): Promise<void> {
    const job = await this.prisma.job.findUnique({
      where: { id: jobId },
      select: { status: true, customerId: true, number: true, type: true, createdAt: true },
    });
    if (!job || job.status !== JobStatus.SEARCHING) return;
    await this.cancelOpenOffers(jobId, 'dispatch_timeout');
    await this.jobs.transition(jobId, JobStatus.NO_PARTNER_AVAILABLE, {
    ...
    await this.notifications.notify({
      ...
      event: NotificationEvent.NO_PARTNER_AVAILABLE,
      jobId,
  ...
  async retry(jobId: string, user: RequestUser): Promise<void> {
    ...
    if (job.status !== JobStatus.NO_PARTNER_AVAILABLE)
      throw AppException.invalidTransition(job.status, JobStatus.SEARCHING);
    await this.prisma.jobAssignment.updateMany({
      where: { jobId, status: { in: [AssignmentStatus.EXPIRED, AssignmentStatus.REJECTED] } },
      data: { releaseReason: 'retry_reset' },
    });
    ...
    await this.prisma.jobAssignment.deleteMany({
      where: {
        jobId,
        status: {
          in: [AssignmentStatus.EXPIRED, AssignmentStatus.REJECTED, AssignmentStatus.CANCELLED],
        },
      },
    });
  ...
  private async cancelOpenOffers(jobId: string, reason: string): Promise<void> {
    const open = await this.prisma.jobAssignment.findMany({
      where: { jobId, status: AssignmentStatus.OFFERED },
    });
    if (!open.length) return;
    await this.prisma.jobAssignment.updateMany({
      where: { id: { in: open.map((o) => o.id) } },
      data: { status: AssignmentStatus.CANCELLED, releasedAt: new Date(), releaseReason: reason },
    });
    for (const o of open)
      this.events.emit(DispatchEvents.OFFER_EXPIRED, {
  ...
  async listOffers(user: RequestUser): Promise<JobOfferDto[]> {
    ...
    const rows = await this.prisma.jobAssignment.findMany({
      where: { partnerId, status: AssignmentStatus.OFFERED, expiresAt: { gt: new Date() } },
      include: { job: { include: jobInclude } },
      orderBy: { offeredAt: 'desc' },
    });
    return rows
      .filter((r) => r.job.status === JobStatus.SEARCHING)
      .map((r) => ({
        assignmentId: r.id,
        job: { ...this.mapper.toDto(r.job, { kind: 'share-link' }), customer: undefined },
        wave: r.wave,
        expiresAt: r.expiresAt.toISOString(),
        distanceToPickupMeters: r.distanceMeters,
        etaToPickupSeconds: r.etaSeconds,
        estimatedEarnings: {
          amount: Number(r.estimatedEarningsMinor),
          currency: r.job.currency as JobOfferDto['estimatedEarnings']['currency'],
        },
      }));
  }
  ...
  async respond(
    ...
    if (!assignment || assignment.partnerId !== partnerId)
      throw AppException.notFound('Offer', assignmentId);
    if (assignment.status !== AssignmentStatus.OFFERED)
      throw AppException.conflict('This offer is no longer available', ErrorCode.OFFER_EXPIRED);
    if (assignment.expiresAt < new Date())
      throw AppException.conflict('This offer has expired', ErrorCode.OFFER_EXPIRED);

    if (!accept) {
      await this.prisma.jobAssignment.updateMany({
        where: { id: assignmentId, status: AssignmentStatus.OFFERED },
        data: { status: AssignmentStatus.REJECTED, respondedAt: new Date() },
      });
      return null;
    }

    if (!(await this.availability.isEffectivelyOnline(partnerId)))
      throw AppException.badRequest(ErrorCode.PARTNER_NOT_AVAILABLE, 'Go online to accept jobs');
    ...
    try {
      result = await this.lock.withLock(`job:${assignment.jobId}`, 5000, () =>
        this.prisma.$transaction(async (tx) => {
          ...
          if (partner.availability?.currentJobId)
            throw AppException.conflict(
              'You already have an active job',
              ErrorCode.JOB_ALREADY_ASSIGNED,
            );
          const t = await this.jobs.transitionInTx(
            tx,
            assignment.jobId,
            JobStatus.ASSIGNED,
            { type: JobActorType.SYSTEM, id: partnerId },
            {
              data: { assignmentId, wave: assignment.wave, score: assignment.score.toNumber() },
              patch: {
                partner: { connect: { userId: partnerId } },
                ...(partner.activeVehicleId
                  ? { vehicle: { connect: { id: partner.activeVehicleId } } }
                  : {}),
                etaToPickupSeconds: assignment.etaSeconds,
              },
            },
          );
          const others = await tx.jobAssignment.findMany({
            where: { jobId: assignment.jobId, status: AssignmentStatus.OFFERED },
          });
          ...
          await tx.partnerProfile.update({
            where: { userId: partnerId },
            data: { offersAccepted: { increment: 1 } },
          });
          await tx.partnerAvailability.update({
            where: { partnerId },
            data: { status: 'BUSY', currentJobId: assignment.jobId },
          });
      ...
      if (err instanceof LockBusyError)
        throw AppException.conflict(
          'Another partner is accepting this job',
          ErrorCode.JOB_ALREADY_ASSIGNED,
        );
      if (err instanceof Prisma.PrismaClientKnownRequestError && err.code === 'P2002')
        throw AppException.conflict('Job was already taken', ErrorCode.JOB_ALREADY_ASSIGNED);
      if (err instanceof AppException && err.code === ErrorCode.INVALID_STATE_TRANSITION)
        throw AppException.conflict('Job was already taken', ErrorCode.JOB_ALREADY_ASSIGNED);
      throw err;
    }
    result.emit();
    ...
    if (job.dispatchStartedAt)
      this.metrics.dispatchSeconds.observe(
        { type: job.type },
        (Date.now() - job.dispatchStartedAt.getTime()) / 1000,
      );
    await this.notifications.notify({
      userId: job.customerId,
      event: NotificationEvent.JOB_ACCEPTED,
      jobId: job.id,
      priority: 'high',
      collapseKey: `job:${job.id}`,
      vars: {
        partnerName: job.partner?.user.fullName ?? '',
        etaMinutes: String(Math.max(1, Math.round((assignment.etaSeconds || 300) / 60))),
      },
    });
    if (location)
      await this.availability
        ...
        .catch((err: unknown) => this.logger.warn({ err }, 'heartbeat after accept failed'));
    return job;
  }
  ...
  async releaseByPartner(jobId: string, user: RequestUser, reason: string): Promise<void> {
    ...
    await this.jobs.transition(
      jobId,
      JobStatus.SEARCHING,
      { type: JobActorType.PARTNER, id: user.id },
      { reason, patch: { dispatchWave: 0 } },
    );
    await this.prisma.$transaction([
      this.prisma.jobAssignment.updateMany({
        where: { jobId, partnerId, status: AssignmentStatus.ACCEPTED },
        data: {
      ...
      this.prisma.partnerProfile.update({
        where: { userId: partnerId },
        data: { cancelledJobs: { increment: 1 }, penaltyPoints: { increment: penalty } },
      }),
    ]);
    await this.availability.setBusy(partnerId, null);
    await this.notifications.notify({
      userId: job.customerId,
      event: NotificationEvent.JOB_CREATED,
      jobId,
      vars: { jobNumber: job.number },
      data: { hint: 'partner_released' },
    });
    await this.runWave(jobId, 1);
  }
  ...
  async manualAssign(
    ...
    const partner = await this.prisma.partnerProfile.findUnique({
      where: { userId: partnerId },
      include: { availability: true, roles: true },
    });
    if (!partner || partner.verificationStatus !== 'APPROVED')
      throw AppException.badRequest(ErrorCode.PARTNER_NOT_APPROVED, 'Partner is not approved');
    if (partner.availability?.currentJobId && partner.availability.currentJobId !== jobId)
      throw AppException.conflict(
        'Partner is busy on another job',
        ErrorCode.PARTNER_NOT_AVAILABLE,
      );
    const requiredRole = this.roleFor(job);
    if (!partner.roles.some((r) => r.isActive && r.role === requiredRole))
      throw AppException.badRequest(
        ErrorCode.PARTNER_NOT_AVAILABLE,
        `Partner lacks the ${requiredRole} role`,
      );

    if (
      job.status === JobStatus.ASSIGNED ||
      job.status === JobStatus.PARTNER_EN_ROUTE ||
      job.status === JobStatus.PARTNER_ARRIVED ||
      ...
      await this.jobs.transition(
        ...
        { reason, requestId, eventType: 'partner.unassigned', expectedVersion },
      );
      if (previous) {
        await this.prisma.jobAssignment.updateMany({
          where: { jobId, partnerId: previous, status: AssignmentStatus.ACCEPTED },
          data: {
            status: AssignmentStatus.REASSIGNED,
            releasedAt: new Date(),
            releaseReason: 'dispatcher_reassign',
          },
        });
        await this.availability.setBusy(previous, null);
        await this.notifications.notify({
          userId: previous,
          event: NotificationEvent.JOB_CANCELLED,
          jobId,
          vars: { jobNumber: job.number, reason: '' },
          priority: 'high',
        });
      }
    } else if (
      job.status === JobStatus.REQUESTED ||
      job.status === JobStatus.NO_PARTNER_AVAILABLE
    ) {
      await this.jobs.transition(
        jobId,
        JobStatus.SEARCHING,
    ...
    const distance =
      pickup && loc?.lat && loc.lng
        ? this.pricing.distanceMeters(
            { lat: pickup.lat.toNumber(), lng: pickup.lng.toNumber() },
            { lat: loc.lat.toNumber(), lng: loc.lng.toNumber() },
          )
        : 0;
    const earnings = await this.pricing.estimatedPartnerEarnings(
      job.estimatedTotalMinor ?? 0n,
    ...
    const result = await this.prisma.$transaction(async (tx) => {
      await tx.jobAssignment.create({
        data: {
          ...
          etaSeconds: estimateEtaSeconds(distance * 1.3),
          estimatedEarningsMinor: earnings,
          isManual: true,
      ...
      const t = await this.jobs.transitionInTx(
        ...
        { type: JobActorType.ADMIN, id: actor.id },
        {
          reason,
          requestId,
          eventType: 'partner.assigned_manually',
          patch: {
            partner: { connect: { userId: partnerId } },
            ...(partner.activeVehicleId
              ? { vehicle: { connect: { id: partner.activeVehicleId } } }
              : {}),
            etaToPickupSeconds: estimateEtaSeconds(distance * 1.3),
          },
        },
      );
      await tx.partnerAvailability.upsert({
        where: { partnerId },
        update: { status: 'BUSY', currentJobId: jobId },
        create: { partnerId, status: 'BUSY', currentJobId: jobId },
      });
      await this.audit.record(
        {
          ...
          entity: 'job',
          entityId: jobId,
          newValue: { partnerId, previousPartnerId: job.partnerId },
          reason,
          requestId,
        },
        tx,
    ...
    await this.notifications.notify({
      ...
      jobId,
      priority: 'high',
      vars: { partnerName: '', etaMinutes: '' },
      data: { hint: 'manual_assignment' },
    });
    await this.notifications.notify({
      userId: job.customerId,
      event: NotificationEvent.JOB_ACCEPTED,
      ...
      priority: 'high',
      vars: {
        partnerName: result.updated.partner?.user.fullName ?? '',
        etaMinutes: String(Math.max(1, Math.round(estimateEtaSeconds(distance * 1.3) / 60))),
      },
    });
    return result.updated;
  }

  async nearbyPartners(q: NearbyPartnersQueryInput): Promise<
    Array<
      LiveMapPartnerDto & {
  ...
  > {
    ...
    return rows.map((r) => ({
      ...
      availability: r.status as LiveMapPartnerDto['availability'],
      roles: r.roles,
      location: { lat: r.lat, lng: r.lng },
      heading: r.heading,
      activeJobId: r.current_job_id,
      lastSeenAt: r.last_heartbeat_at.toISOString(),
      distanceMeters: Math.round(r.distance_m),
      rating: r.rating_count ? Number((r.rating_sum / r.rating_count).toFixed(2)) : 5,
  ...
  async assignmentsForJob(jobId: string) {
    return this.prisma.jobAssignment.findMany({
      where: { jobId },
      include: { partner: { include: { user: { select: { fullName: true, phone: true } } } } },
      orderBy: [{ wave: 'asc' }, { score: 'desc' }],
    });
  }
  ...
  private async waveParams(wave: number): Promise<{ radius: number; size: number }> {
    const keys =
      wave === 1
        ? [CONFIG_KEYS.DISPATCH_WAVE_1_RADIUS_M, CONFIG_KEYS.DISPATCH_WAVE_1_SIZE]
        : wave === 2
          ? [CONFIG_KEYS.DISPATCH_WAVE_2_RADIUS_M, CONFIG_KEYS.DISPATCH_WAVE_2_SIZE]
          : [CONFIG_KEYS.DISPATCH_WAVE_3_RADIUS_M, CONFIG_KEYS.DISPATCH_WAVE_3_SIZE];
    const [radiusKey, sizeKey] = keys as [
      typeof CONFIG_KEYS.DISPATCH_WAVE_1_RADIUS_M,
      typeof CONFIG_KEYS.DISPATCH_WAVE_1_SIZE,
    ];
    const [radius, size] = await Promise.all([
      this.config.getNumber(radiusKey),
      this.config.getNumber(sizeKey),
    ]);
  ...
  private async findCandidates(
    ...
    const [heartbeatMax, maxNegative] = await Promise.all([
      this.config.getNumber(CONFIG_KEYS.HEARTBEAT_OFFLINE_AFTER_S),
      this.config.getNumber(CONFIG_KEYS.WALLET_MAX_NEGATIVE_PARTNER_MINOR),
    ]);
  ...
  private async score(job: JobWithRelations, rows: CandidateRow[], radius: number) {
    ...
    const pickup = job.stops[0];
    const weights = await this.config.getMany([
      CONFIG_KEYS.DISPATCH_SCORE_W_ETA,
      CONFIG_KEYS.DISPATCH_SCORE_W_DISTANCE,
      CONFIG_KEYS.DISPATCH_SCORE_W_RATING,
      CONFIG_KEYS.DISPATCH_SCORE_W_ACCEPTANCE,
      CONFIG_KEYS.DISPATCH_SCORE_W_CANCELLATION,
      CONFIG_KEYS.DISPATCH_SCORE_W_WORKLOAD,
    ]);
    let etas: Array<number | null> = rows.map((r) =>
      estimateEtaSeconds(Number(r.distance_m) * 1.3),
    );
    if (pickup && rows.length <= 25) {
      try {
        const matrix = await this.maps.distanceMatrix(
    ...
    const inputs: CandidateInput[] = rows.map((r, i) => ({
      ...
      cancellationRate:
        r.completed_jobs + r.cancelled_jobs
          ? r.cancelled_jobs / (r.completed_jobs + r.cancelled_jobs)
          : 0,
    ...
    return scoreCandidates(
      inputs,
      {
        eta: Number(weights[CONFIG_KEYS.DISPATCH_SCORE_W_ETA]),
        distance: Number(weights[CONFIG_KEYS.DISPATCH_SCORE_W_DISTANCE]),
        rating: Number(weights[CONFIG_KEYS.DISPATCH_SCORE_W_RATING]),
        acceptance: Number(weights[CONFIG_KEYS.DISPATCH_SCORE_W_ACCEPTANCE]),
        cancellation: Number(weights[CONFIG_KEYS.DISPATCH_SCORE_W_CANCELLATION]),
        workload: Number(weights[CONFIG_KEYS.DISPATCH_SCORE_W_WORKLOAD]),
      },
      radius,
    );
  }
  ...
  private roleFor(job: {
    type: JobType;
    category?: { requiredPartnerRole: PartnerRoleType } | null;
  }): PartnerRoleType {
