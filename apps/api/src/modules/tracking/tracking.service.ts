...
const BEFORE_START_STATUSES: readonly JobStatus[] = [
  JobStatus.ASSIGNED,
...
export class TrackingService {
  ...
  async ingestForPartner(
    ...
    const [maxStale, maxAccuracy, maxSpeedKmh] = await Promise.all([
      this.config.getNumber(CONFIG_KEYS.TRACKING_MAX_STALE_S),
      this.config.getNumber(CONFIG_KEYS.TRACKING_MAX_ACCURACY_M),
      this.config.getNumber(CONFIG_KEYS.TRACKING_MAX_SPEED_KMH),
    ]);
    const availability = await this.prisma.partnerAvailability.findUnique({
      where: { partnerId },
      select: { currentJobId: true, status: true },
    });
    const jobId = availability?.currentJobId ?? null;
    if (jobIdHint && jobId && jobIdHint !== jobId)
      throw AppException.forbidden('Location does not belong to your active job');
    ...
    for (const s of ordered) {
      ...
      if (Number.isNaN(ts) || now - ts > maxStale * 1000 || ts - now > 30_000) {
        rejected += 1;
        this.metrics.locationUpdates.inc({ result: 'stale' });
        continue;
      }
      if (s.accuracy > maxAccuracy) {
        rejected += 1;
        this.metrics.locationUpdates.inc({ result: 'inaccurate' });
        continue;
      }
      if (previous) {
        ...
        if (dt <= 0) {
          rejected += 1;
          this.metrics.locationUpdates.inc({ result: 'out_of_order' });
          continue;
        }
        ...
        if (kmh > maxSpeedKmh && meters > 200) {
          ...
          this.events.emit(TrackingEvents.IMPOSSIBLE_MOVEMENT, {
            partnerId,
            jobId,
            kmh: Math.round(kmh),
          ...
          await this.risk.recordSignal(
            partnerId,
            RiskSignal.IMPOSSIBLE_GPS_MOVEMENT,
            Math.min(100, Math.round(kmh / 10)),
            { kmh: Math.round(kmh), meters: Math.round(meters), seconds: Math.round(dt) },
            jobId ?? undefined,
          );
          continue;
        }
    ...
    if (!accepted.length)
      return { accepted: 0, rejected, etaToPickupSeconds: null, etaToDestinationSeconds: null };
    ...
    await this.prisma.partnerAvailability.update({
      where: { partnerId },
      data: {
        lat: new Prisma.Decimal(latest.lat),
        lng: new Prisma.Decimal(latest.lng),
        heading: latest.heading === undefined ? null : new Prisma.Decimal(latest.heading),
        speed: latest.speed === undefined ? null : new Prisma.Decimal(latest.speed),
        accuracy: new Prisma.Decimal(latest.accuracy),
        lastLocationAt: new Date(latest.timestamp),
        lastHeartbeatAt: new Date(),
      },
    });

    let eta: { etaToPickupSeconds: number | null; etaToDestinationSeconds: number | null } = {
      etaToPickupSeconds: null,
      etaToDestinationSeconds: null,
    };
    if (jobId) {
      await this.prisma.jobTrackingPoint.createMany({
        data: accepted.map((s) => ({
          jobId,
          partnerId,
          lat: new Prisma.Decimal(s.lat),
          lng: new Prisma.Decimal(s.lng),
          accuracy: new Prisma.Decimal(s.accuracy),
          heading: s.heading === undefined ? null : new Prisma.Decimal(s.heading),
          speed: s.speed === undefined ? null : new Prisma.Decimal(s.speed),
  ...
  async latestPartnerLocation(partnerId: string): Promise<PartnerLocationState | null> {
    ...
    return {
      ...
      accuracy: row.accuracy?.toNumber() ?? 0,
      timestamp: row.lastLocationAt.toISOString(),
  ...
  private async maybeRefreshEta(
    jobId: string,
    from: GeoPoint,
  ): Promise<{ etaToPickupSeconds: number | null; etaToDestinationSeconds: number | null }> {
  ...
  private async computeEta(
    job: {
      id: string;
      status: JobStatus;
      stops: Array<{ kind: string; lat: Prisma.Decimal; lng: Prisma.Decimal }>;
    },
    from: GeoPoint,
  ): Promise<{ etaToPickupSeconds: number | null; etaToDestinationSeconds: number | null }> {
    ...
    try {
      if (beforeStart && pickup) {
        const r = await this.maps.route(from, {
          lat: pickup.lat.toNumber(),
          lng: pickup.lng.toNumber(),
        });
        ...
        const r = await this.maps.route(from, {
          lat: dest.lat.toNumber(),
          lng: dest.lng.toNumber(),
        });
    ...
    if (etaToPickupSeconds !== null || etaToDestinationSeconds !== null) {
      await this.prisma.job.update({
        where: { id: job.id },
        data: {
          etaToPickupSeconds: etaToPickupSeconds ?? undefined,
          etaToDestinationSeconds: etaToDestinationSeconds ?? undefined,
        },
      });
  ...
  async jobActuals(
    ...
    const points = await this.prisma.jobTrackingPoint.findMany({
      where: { jobId, recordedAt: { gte: since } },
    ...
    for (let i = 1; i < points.length; i += 1) {
      ...
      const d = haversineMeters(
        ...
        { lat: b.lat.toNumber(), lng: b.lng.toNumber() },
      );
    ...
    const duration =
      first && last
        ? Math.round((last.recordedAt.getTime() - first.recordedAt.getTime()) / 1000)
  ...
  async liveMap(
    q: LiveMapQueryInput,
  ): Promise<{ partners: LiveMapPartnerDto[]; jobs: LiveMapJobDto[] }> {
    ...
    const partners: LiveMapPartnerDto[] = q.includePartners
      ? (
          ...
          roles: r.roles as LiveMapPartnerDto['roles'],
      ...
      ? (
          ...
          .map((j) => {
            ...
            return {
              ...
              type: j.type,
              status: j.status,
              pickup: p ? { lat: p.lat.toNumber(), lng: p.lng.toNumber() } : { lat: 0, lng: 0 },
              destination: d && d !== p ? { lat: d.lat.toNumber(), lng: d.lng.toNumber() } : null,
  ...
  async purgeOldPoints(): Promise<number> {
    ...
    const res = await this.prisma.jobTrackingPoint.deleteMany({
  ...
  async intervalHint(hasActiveJob: boolean): Promise<number> {
    return this.config.getNumber(
      hasActiveJob ? CONFIG_KEYS.TRACKING_INTERVAL_ACTIVE_S : CONFIG_KEYS.TRACKING_INTERVAL_IDLE_S,
    );
  }

  assertNotStaleForArrival(sample: LocationSample, maxStaleSeconds: number): void {
    if (Date.now() - Date.parse(sample.timestamp) > maxStaleSeconds * 1000)
      throw AppException.badRequest(ErrorCode.STALE_LOCATION, 'Location sample is too old');
  }
}
