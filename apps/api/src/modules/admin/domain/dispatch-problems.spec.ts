...
describe('classifyProblems', () => {
  ...
  it('flags an exhausted search separately from a fresh one', () => {
    const problems = classifyProblems(
      input({ status: JobStatus.NO_PARTNER_AVAILABLE, partnerId: null, assignedAt: null }),
      DEFAULT_PROBLEM_THRESHOLDS,
      NOW,
    );
  ...
  it('flags a partner who accepted but has not started moving after the grace window', () => {
    const justInside = classifyProblems(
      input({ assignedAt: secondsAgo(DEFAULT_PROBLEM_THRESHOLDS.assignedGraceSeconds) }),
      DEFAULT_PROBLEM_THRESHOLDS,
      NOW,
    );
    ...
    const late = classifyProblems(
      input({ assignedAt: secondsAgo(DEFAULT_PROBLEM_THRESHOLDS.assignedGraceSeconds + 1) }),
      DEFAULT_PROBLEM_THRESHOLDS,
      NOW,
    );
  ...
  it('flags an en-route job only after twice the promised ETA', () => {
    const onTime = classifyProblems(
      input({
        status: JobStatus.PARTNER_EN_ROUTE,
        enRouteAt: secondsAgo(500),
        etaToPickupSeconds: 300,
      }),
      DEFAULT_PROBLEM_THRESHOLDS,
      NOW,
  ...
  it('always flags a job waiting on the customer', () => {
    expect(
      classifyProblems(
        input({ status: JobStatus.WAITING_CUSTOMER }),
        DEFAULT_PROBLEM_THRESHOLDS,
        NOW,
      ),
    ).toContain(DispatchProblem.WAITING_CUSTOMER);
  });

  it('flags a silent partner device', () => {
    expect(
      classifyProblems(
        input({ partnerLastHeartbeatAt: secondsAgo(121) }),
        DEFAULT_PROBLEM_THRESHOLDS,
        NOW,
      ),
    ).toContain(DispatchProblem.PARTNER_HEARTBEAT_STALE);
    expect(
      classifyProblems(input({ partnerLastHeartbeatAt: null }), DEFAULT_PROBLEM_THRESHOLDS, NOW),
    ).toContain(DispatchProblem.PARTNER_HEARTBEAT_STALE);
  });
  ...
  it('can report several problems at once', () => {
    ...
    expect(problems).toEqual([
      DispatchProblem.WAITING_CUSTOMER,
      DispatchProblem.PARTNER_HEARTBEAT_STALE,
    ]);
  });
});
