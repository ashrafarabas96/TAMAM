...
export class MaintenanceScheduler {
  ...
  async everyMinute(): Promise<void> {
    ...
    await Promise.all([this.enqueue(MAINTENANCE_JOBS.HEARTBEAT_SWEEP, stamp), this.enqueue(MAINTENANCE_JOBS.CAMPAIGN_SCHEDULER, stamp)]);
  }

  @Cron('*/10 * * * *', { name: 'maintenance.ten-minutes', timeZone: 'UTC' })
  async everyTenMinutes(): Promise<void> {
    const stamp = minuteStamp();
    await Promise.all([this.enqueue(MAINTENANCE_JOBS.EXPIRE_OTPS, stamp), this.enqueue(MAINTENANCE_JOBS.SESSION_CLEANUP, stamp)]);
  }
  ...
  async hourly(): Promise<void> {
    ...
    await Promise.all([this.enqueue(MAINTENANCE_JOBS.BANNER_STATS_ROLLUP, stamp, { date: today }), this.enqueue(MAINTENANCE_JOBS.TRACKING_RETENTION, stamp)]);
  }
  ...
  async runNow(name: MaintenanceJobName, triggeredBy: string, date?: string): Promise<{ jobId: string }> {
    const jobId = `${name}-manual-${Date.now()}`;
    await this.queue.add(name, { manual: true, triggeredBy, ...(date ? { date } : {}) }, { jobId, attempts: 1 });
    this.logger.info({ job: name, jobId, triggeredBy }, 'maintenance job triggered manually');
    return { jobId };
  }

  private async enqueue(name: MaintenanceJobName, stamp: string, data: MaintenanceJobData = {}): Promise<void> {
    await this.queue.add(name, data, { jobId: `${name}-${stamp}`, attempts: 3, removeOnComplete: 200, removeOnFail: 500 });
  }
}
