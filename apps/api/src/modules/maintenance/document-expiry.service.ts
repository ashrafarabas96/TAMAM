...
export class DocumentExpiryService {
  ...
  async run(now: Date = new Date()): Promise<DocumentExpiryResult> {
    ...
    const partnersForcedOffline = expiredIds.partnerIds.length
      ? await this.forceOffline(expiredIds.partnerIds, now)
      : 0;

    this.logger.info(
      { warned, expired: expiredIds.count, partnersForcedOffline },
      'partner document expiry sweep finished',
    );
  ...
  private async warn(candidates: ExpiringDocument[], now: Date): Promise<number> {
    ...
    for (const doc of due) {
      ...
      const claimed = await this.prisma.partnerDocument.updateMany({
        where: { id: doc.id, expiryNotifiedAt: null },
      ...
      try {
        await this.notifications.notify({
          ...
          vars: {
            ...
            days: String(daysUntil(doc.expiresAt, now)),
          },
        ...
        this.logger.error(
          ...
          'could not queue DOCUMENT_EXPIRING notification',
        );
        throw err;
      }
    }
  ...
  private async expire(
    candidates: ExpiringDocument[],
    now: Date,
  ): Promise<{ count: number; partnerIds: string[] }> {
  ...
  private async forceOffline(partnerIds: string[], now: Date): Promise<number> {
    ...
    for (const partner of partners) {
      ...
      if (!partner.availability || partner.availability.status === AvailabilityStatus.OFFLINE)
        continue;
      ...
      this.logger.warn(
        { partnerId: partner.userId, expired },
        'partner forced offline — required documents expired',
      );
      forced += 1;
    }
    return forced;
