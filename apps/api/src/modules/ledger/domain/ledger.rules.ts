...
export const platformAccountCode = (type: LedgerAccountType, currency: string): string =>
  `${type}:${currency}`;

export const walletAccountCode = (walletId: string): string => `WALLET:${walletId}`;

const debit = (accountCode: string, amountMinor: bigint): LedgerLine => ({
  accountCode,
  direction: LedgerEntryDirection.DEBIT,
  amountMinor,
});
const credit = (accountCode: string, amountMinor: bigint): LedgerLine => ({
  accountCode,
...
export function assertBalanced(entries: readonly LedgerLine[]): void {
  if (entries.length < 2) {
    throw AppException.validation(
      [{ field: 'entries', message: 'a ledger transaction needs at least two entries' }],
  ...
  entries.forEach((entry, index) => {
    if (entry.amountMinor <= 0n) {
      throw AppException.validation(
        [{ field: `entries.${index}.amountMinor`, message: 'entry amount must be positive' }],
        'Invalid ledger entry',
      );
    }
    if (!entry.accountCode && !entry.walletId) {
      throw AppException.validation(
        [{ field: `entries.${index}`, message: 'entry needs accountCode or walletId' }],
        'Invalid ledger entry',
      );
    }
    if (entry.accountCode && entry.walletId) {
      throw AppException.validation(
        [
          {
            field: `entries.${index}`,
            message: 'entry accepts either accountCode or walletId, not both',
          },
        ],
        'Invalid ledger entry',
      );
...
export function settlementEntries(input: SettlementInput): SettlementPlan {
  ...
  const gatewayClearing = platformAccountCode(
  ...
  if (grossFareMinor > 0n) {
    if (isCash) {
      if (input.jobTotalMinor > 0n) {
        ...
        entries.push(
          debit(cashClearing, input.jobTotalMinor),
          credit(partnerWalletAccountCode, input.jobTotalMinor),
        );
      }
      if (input.promoDiscountMinor > 0n) {
        entries.push(
          debit(promoExpense, input.promoDiscountMinor),
          credit(partnerWalletAccountCode, input.promoDiscountMinor),
        );
      }
      if (commissionMinor > 0n) {
        entries.push(
          debit(partnerWalletAccountCode, commissionMinor),
          credit(revenue, commissionMinor),
        );
      }
      if (input.jobTotalMinor > 0n) {
        entries.push(
          debit(partnerWalletAccountCode, input.jobTotalMinor),
          credit(cashClearing, input.jobTotalMinor),
        );
      }
    } else {
      if (input.jobTotalMinor > 0n) {
        if (isWallet) {
          if (!input.customerWalletAccountCode) {
            throw AppException.validation(
              [
                {
                  field: 'customerWalletAccountCode',
                  message: 'wallet settlements need the customer wallet account',
                },
              ],
              'Invalid settlement input',
            );
      ...
      if (input.promoDiscountMinor > 0n)
        entries.push(debit(promoExpense, input.promoDiscountMinor));
  ...
  const partnerCancellationCompensationMinor = clamp(
    input.partnerFeeOnCancelMinor ?? 0n,
    0n,
    input.cancellationFeeMinor,
  );
  if (input.cancellationFeeMinor > 0n) {
      ...
      if (!input.customerWalletAccountCode) {
        throw AppException.validation(
          [
            {
              field: 'customerWalletAccountCode',
              message: 'cancellation fees on cash/wallet jobs are charged to the customer wallet',
            },
          ],
          'Invalid settlement input',
        );
    ...
    if (partnerCancellationCompensationMinor > 0n)
      entries.push(credit(partnerWalletAccountCode, partnerCancellationCompensationMinor));
  ...
  return {
    type:
      grossFareMinor > 0n
        ? LedgerTransactionType.JOB_CHARGE
        : LedgerTransactionType.CANCELLATION_FEE,
    entries,
    grossFareMinor,
    commissionMinor,
    partnerNetMinor,
...
export function refundEntries(params: {
  ...
  const entries: LedgerLine[] = [
    debit(
      platformAccountCode(LedgerAccountType.PLATFORM_REFUND_EXPENSE, params.currency),
      params.amountMinor,
    ),
