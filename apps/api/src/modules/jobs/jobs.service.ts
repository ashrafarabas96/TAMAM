...
export class JobsService {
  ...
  async getRaw(jobId: string, tx?: Tx): Promise<JobWithRelations> {
    const job = await (tx ?? this.prisma).job.findUnique({
      where: { id: jobId },
      include: jobInclude,
    });
  ...
  async getDto(jobId: string, user: RequestUser, withTimeline = false): Promise<JobDto> {
    ...
    const events = withTimeline
      ? await this.prisma.jobEvent.findMany({ where: { jobId }, orderBy: { createdAt: 'asc' } })
      : undefined;
  ...
  async listForUser(
    ...
    const statusGroup =
      filter.statusGroup === 'active'
        ...
        : filter.statusGroup === 'completed'
          ? { in: [JobStatus.COMPLETED] }
          : filter.statusGroup === 'cancelled'
            ? { in: [JobStatus.CANCELLED, JobStatus.NO_PARTNER_AVAILABLE] }
            : undefined;
    const rows = await this.prisma.job.findMany({
      where: {
        ...cursorWhere(cursor),
        ...(isStaff
          ? { customerId: filter.customerId, partnerId: filter.partnerId, zoneId: filter.zoneId }
          : user.partnerId && user.roles.includes('PARTNER') && !user.roles.includes('CUSTOMER')
        ...
        ...(filter.q
          ? {
              OR: [
                { number: { contains: filter.q.toUpperCase() } },
                { customer: { user: { phone: { contains: filter.q } } } },
              ],
            }
          : {}),
      },
  ...
  async create(
    ...
    if (est.jobType !== input.type)
      throw AppException.validation([
        { field: 'estimateId', message: 'estimate does not match job type' },
      ]);
    ...
    if (active >= maxActive)
      throw AppException.conflict(`You already have ${active} active requests`, ErrorCode.CONFLICT);

    if (input.scheduling === SchedulingMode.SCHEDULED) {
      await this.sysConfig.assertEnabled(FEATURE_FLAGS.SCHEDULED_JOBS, {
        userId: user.id,
        zoneId: est.zoneId,
      });
      if (!input.scheduledFor || new Date(input.scheduledFor).getTime() < Date.now() + 15 * 60_000)
        throw AppException.validation([
          { field: 'scheduledFor', message: 'must be at least 15 minutes in the future' },
        ]);
    }
    ...
    const optionKey =
      input.type === JobType.HOME_SERVICE
        ? (est.categoryId ?? '')
        : ((input as { vehicleTypeId?: string }).vehicleTypeId ?? '');
    if (!optionKey)
      throw AppException.validation([{ field: 'vehicleTypeId', message: 'required' }]);
    ...
    if (input.promoCode) {
      ...
      const result = await this.promotions.evaluate(input.promoCode, {
        userId: user.id,
        jobType: est.jobType,
        categoryId: est.categoryId,
        zoneId: est.zoneId,
        paymentMethod: input.paymentMethod,
        subtotalMinor: BigInt(option?.subtotalMinor ?? '0'),
    ...
    if (input.type === JobType.HOME_SERVICE) {
      ...
      if (est.categoryId !== input.categoryId)
        throw AppException.validation([
      ...
      if (images < reqMedia.minImages)
        throw AppException.validation([
          { field: 'mediaIds', message: `at least ${reqMedia.minImages} photo(s) required` },
        ]);
      if (images > reqMedia.maxImages)
        throw AppException.validation([
          { field: 'mediaIds', message: `at most ${reqMedia.maxImages} photos` },
        ]);
    }
    if (mediaIds.length)
      await this.media.assertOwnedReady(user.id, mediaIds, [MediaPurpose.JOB_ATTACHMENT]);

    const [tripPinEnabled, pickupOtpEnabled, deliveryOtpEnabled] = await Promise.all([
      this.sysConfig.getBoolean(CONFIG_KEYS.JOB_TRIP_PIN_ENABLED).then(
        async (v) =>
          v &&
          (await this.sysConfig.isEnabled(FEATURE_FLAGS.TRIP_PIN, {
            userId: user.id,
            zoneId: est.zoneId,
          })),
      ),
    ...
    const deliveryOtp =
      input.type === JobType.DELIVERY && deliveryOtpEnabled ? randomDigits(4) : null;

    const jobId = await this.prisma.$transaction(async (tx) => {
      const snapshot = await this.pricing.createSnapshot(
        est,
        optionKey,
        promo?.discountMinor ?? 0n,
        tx,
      );
      ...
      const addr = (
      ...
      ): Prisma.JobStopCreateWithoutJobInput => ({
        sequence,
        kind,
        ...
        contactPhoneEnc: contact ? encrypt(contact.phone, key) : null,
      });
      if (input.type === JobType.RIDE) {
        stops.push(
          addr(input.pickup, 0, JobStopKind.PICKUP),
          addr(input.destination, 1, JobStopKind.DROPOFF),
        );
      } else if (input.type === JobType.DELIVERY) {
        stops.push(
          addr(input.pickup, 0, JobStopKind.PICKUP, input.sender),
          addr(input.destination, 1, JobStopKind.DROPOFF, input.recipient),
        );
      } else {
      ...
      const job = await tx.job.create({
        data: {
          ...
          scheduledFor:
            input.scheduling === SchedulingMode.SCHEDULED && input.scheduledFor
          ...
          preferredDate:
            input.type === JobType.HOME_SERVICE && input.preferredDate
              ? new Date(input.preferredDate)
              : null,
          preferredTimeSlot:
            input.type === JobType.HOME_SERVICE ? (input.preferredTimeSlot ?? null) : null,
          ...
          media: mediaIds.length
            ...
            : undefined,
          serviceOptions:
            input.type === JobType.HOME_SERVICE && input.optionIds.length && category
              ? {
                  create: input.optionIds.map((optionId) => {
                    const o = category?.subcategories
                      .flatMap((s) => s.options)
                      .find((x) => x.id === optionId);
                    return { optionId, priceMinor: o?.priceMinor ?? 0n };
                  }),
                }
              : undefined,
          delivery:
            input.type === JobType.DELIVERY && est.packageCategoryId
              ? {
                  create: {
                    packageCategoryId: est.packageCategoryId,
                    approximateSize: input.approximateSize,
                    approximateWeightKg: input.approximateWeightKg ?? null,
                    senderName: input.sender.name,
                    senderPhoneEnc: encrypt(input.sender.phone, key),
                    recipientName: input.recipient.name,
                    recipientPhoneEnc: encrypt(input.recipient.phone, key),
        ...
        data: [
          {
            ...
            actorType: JobActorType.CUSTOMER,
            actorId: user.id,
            data: { estimateId: est.id, optionKey } as Prisma.InputJsonValue,
            requestId,
          },
          {
            jobId: job.id,
      ...
      await this.payments.createForJob(
        {
          id: job.id,
          customerId: user.id,
      ...
      await tx.customerProfile.updateMany({
        where: { userId: user.id, firstJobAt: null },
        data: { firstJobAt: now },
      });
      return job.id;
    });
    ...
    this.events.emit(JobDomainEvents.CREATED, {
      jobId,
      jobType: input.type,
      customerId: user.id,
      zoneId: est.zoneId,
    ...
    if (input.scheduling === SchedulingMode.SCHEDULED && input.scheduledFor) {
      ...
      const delay = Math.max(
        0,
        new Date(input.scheduledFor).getTime() - lead * 60_000 - Date.now(),
      );
      await this.dispatchQueue.add(
        DISPATCH_JOBS.SCHEDULED_KICKOFF,
        { jobId },
        { delay, jobId: `kickoff-${jobId}` },
      );
    } else {
      await this.dispatchQueue.add(
        DISPATCH_JOBS.WAVE,
  ...
  private async assertPaymentMethod(
    ...
    if (method === PaymentMethod.CARD || method === PaymentMethod.EXTERNAL_GATEWAY) {
      ...
      if (this.config.env.PAYMENT_GATEWAY_PROVIDER === 'none')
        throw AppException.badRequest(
          ErrorCode.PAYMENT_METHOD_DISABLED,
          'Online payments are not enabled',
        );
    } else if (method === PaymentMethod.WALLET) {
      await this.sysConfig.assertEnabled(FEATURE_FLAGS.WALLET_PAYMENTS, {
        userId: user.id,
        zoneId,
      });
      ...
      throw AppException.badRequest(
        ErrorCode.PAYMENT_METHOD_DISABLED,
        'Bank transfer is not available for jobs',
      );
    }
  }
  ...
  async transition(
    ...
    const result = await this.prisma.$transaction((tx) =>
      this.transitionInTx(tx, jobId, to, actor, opts),
    );
    result.emit();
  ...
  async transitionInTx(
    ...
    actor: TransitionActor,
    opts: TransitionOptions = {},
  ): Promise<{ updated: JobWithRelations; from: JobStatus; emit: () => void }> {
    const locked = await tx.$queryRaw<
      Array<{ id: string; type: JobType; status: JobStatus; version: number }>
    >`SELECT id, type, status, version FROM jobs WHERE id = ${jobId}::uuid FOR UPDATE`;
    ...
    if (to === JobStatus.SEARCHING && !opts.patch?.dispatchStartedAt)
      timestamps.dispatchStartedAt = now;
    if (to === JobStatus.ASSIGNED) timestamps.assignedAt = now;
    if (to === JobStatus.PARTNER_ARRIVED) timestamps.partnerArrivedAt = now;
    if (to === JobStatus.IN_PROGRESS || to === JobStatus.INSPECTION_STARTED)
    ...
    if (
      to === JobStatus.SEARCHING &&
      (current.status === JobStatus.ASSIGNED ||
        current.status === JobStatus.PARTNER_EN_ROUTE ||
        current.status === JobStatus.PARTNER_ARRIVED ||
        current.status === JobStatus.WAITING_CUSTOMER)
    ) {
    ...
    const emit = () => {
      const payload: JobStatusChangedEvent = {
        jobId,
      ...
      if (to === JobStatus.NO_PARTNER_AVAILABLE)
        this.events.emit(JobDomainEvents.NO_PARTNER, payload);
      if (to === JobStatus.IN_PROGRESS || to === JobStatus.INSPECTION_STARTED)
        this.events.emit(JobDomainEvents.STARTED, payload);
      if (
        to === JobStatus.SEARCHING &&
        from !== JobStatus.REQUESTED &&
        from !== JobStatus.NO_PARTNER_AVAILABLE
      )
        this.events.emit(JobDomainEvents.UNASSIGNED, payload);
    };
  ...
  async addEvent(
    ...
    await (tx ?? this.prisma).jobEvent.create({
      data: {
        jobId,
        type,
        actorType: actor.type,
