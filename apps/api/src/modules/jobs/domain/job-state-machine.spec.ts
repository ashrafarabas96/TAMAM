...
describe('JobStateMachine', () => {
  it('allows the canonical ride path', () => {
    ...
    const actors: JobActorType[] = [
      JobActorType.CUSTOMER,
      JobActorType.SYSTEM,
      JobActorType.SYSTEM,
      JobActorType.PARTNER,
      JobActorType.PARTNER,
      JobActorType.PARTNER,
      JobActorType.PARTNER,
    ];
  ...
  it('rejects ASSIGNED -> IN_PROGRESS directly (spec §18)', () => {
    expect(() =>
      JobStateMachine.assert(
        { type: JobType.RIDE, status: JobStatus.ASSIGNED },
        JobStatus.IN_PROGRESS,
        JobActorType.PARTNER,
      ),
    ).toThrow();
  ...
  it('rejects mobility-only transitions for home services', () => {
    expect(
      JobStateMachine.can(
        { type: JobType.HOME_SERVICE, status: JobStatus.PARTNER_ARRIVED },
        JobStatus.IN_PROGRESS,
        JobActorType.PARTNER,
      ),
    ).toBe(false);
  ...
  it('forbids customers from completing jobs', () => {
    expect(
      JobStateMachine.can(
        { type: JobType.RIDE, status: JobStatus.IN_PROGRESS },
        JobStatus.COMPLETED,
        JobActorType.CUSTOMER,
      ),
    ).toBe(false);
  ...
  it('requires quote approval before work starts', () => {
    expect(
      JobStateMachine.can(
        { type: JobType.HOME_SERVICE, status: JobStatus.QUOTE_SUBMITTED },
    ...
    expect(
      JobStateMachine.can(
        { type: JobType.HOME_SERVICE, status: JobStatus.QUOTE_APPROVED },
        JobStatus.WORK_STARTED,
        JobActorType.PARTNER,
      ),
    ).toBe(true);
  ...
  it('never leaves a terminal state except COMPLETED -> DISPUTED', () => {
    expect(
      JobStateMachine.targets(
        { type: JobType.RIDE, status: JobStatus.CANCELLED },
        JobActorType.ADMIN,
      ),
    ).toEqual([]);
    expect(
      JobStateMachine.targets(
        { type: JobType.RIDE, status: JobStatus.COMPLETED },
        JobActorType.CUSTOMER,
      ),
    ).toEqual([JobStatus.DISPUTED]);
  });
});
