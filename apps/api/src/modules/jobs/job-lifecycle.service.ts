...
export class JobLifecycleService {
  ...
  async cancel(
    ...
    if (!JobStateMachine.can(job, JobStatus.CANCELLED, actorType))
      throw AppException.invalidTransition(job.status, JobStatus.CANCELLED);
    if (
      actorType === JobActorType.PARTNER &&
      (input.reasonCode === 'CUSTOMER_NO_SHOW' || input.reasonCode === 'CUSTOMER_UNREACHABLE')
    )
      await this.assertNoShowAllowed(job);

    const fee = await this.pricing.cancellationFee(job, actorType);
    const updated = await this.jobs.transition(
      jobId,
      JobStatus.CANCELLED,
      { type: actorType, id: user.id },
      {
        expectedVersion: input.version,
        reason: input.reasonCode,
        data: {
          reasonText: input.reasonText ?? null,
          customerFeeMinor: Number(fee.customerFeeMinor),
          partnerCompensationMinor: Number(fee.partnerCompensationMinor),
        ...
        patch: {
          ...
          breakdown: (fee.customerFeeMinor > 0n
            ? [
                {
                  code: 'CANCELLATION_FEE',
                  label: { ar: 'رسوم الإلغاء', en: 'Cancellation fee' },
                  amount: { amount: Number(fee.customerFeeMinor), currency: job.currency },
                },
              ]
  ...
  async cancelBySystem(
    ...
    const updated = await this.jobs.transition(jobId, JobStatus.CANCELLED, actor, {
      reason: reasonCode,
      data: { reasonText: reasonText ?? null },
      patch: {
  ...
  private async afterCancellation(
    ...
    await this.prisma.$transaction(async (tx) => {
      ...
      await tx.customerProfile.update({
        ...
        data: { cancelledJobs: { increment: actorType === JobActorType.CUSTOMER ? 1 : 0 } },
      });
      if (job.partnerId) {
        await tx.partnerProfile.update({
          where: { userId: job.partnerId },
          data: {
            cancelledJobs: { increment: actorType === JobActorType.PARTNER ? 1 : 0 },
            penaltyPoints: { increment: fee.partnerPenaltyPoints },
          },
        });
      }
    });
    ...
    if (fee.customerFeeMinor > 0n) {
      try {
        ...
        this.logger.warn(
          { err, jobId: job.id },
          'cancellation fee capture failed — will be retried by finance queue',
        );
      }
    }
    ...
    if (job.partnerId && actorType !== JobActorType.PARTNER)
      await this.notifications.notify({
    ...
    this.logger.info(
      { jobId: job.id, actorType, by: user?.id ?? 'system', fee: fee.customerFeeMinor.toString() },
      'job cancelled',
    );
  }
  ...
  private async assertNoShowAllowed(job: JobWithRelations): Promise<void> {
    ...
    if (!job.partnerArrivedAt || Date.now() - job.partnerArrivedAt.getTime() < waitSeconds * 1000) {
      throw AppException.conflict(
        `You can report a no-show ${Math.ceil(waitSeconds / 60)} minutes after arriving`,
  ...
  async enRoute(
    ...
    const updated = await this.jobs.transition(
      ...
      { expectedVersion: input.version, requestId },
    );
    const eta = await this.tracking.refreshEta(updated);
    await this.notifications.notify({
      userId: job.customerId,
      event: NotificationEvent.PARTNER_ARRIVING,
      jobId,
      vars: {
        partnerName: job.partner?.user.fullName ?? '',
        etaMinutes: String(Math.max(1, Math.round((eta ?? 300) / 60))),
      },
      priority: 'high',
      collapseKey: `job:${jobId}`,
    });
    return this.jobs.toDto(await this.jobs.getRaw(jobId), user);
  }
  ...
  async arrive(
    ...
    const distance = haversineMeters(
      { lat: stop.lat.toNumber(), lng: stop.lng.toNumber() },
      input.location,
    );
    if (distance > geofence + Math.min(input.location.accuracy, 100)) {
      throw AppException.conflict(
        `You are ${Math.round(distance)} m from the pickup point`,
        ErrorCode.VALIDATION_FAILED,
        { distanceMeters: Math.round(distance), geofenceMeters: geofence },
      );
    }
    ...
    const updated = await this.jobs.transition(
      ...
      { type: JobActorType.PARTNER, id: user.id },
      {
        ...
        data: { distanceMeters: Math.round(distance) },
      },
    );
    await this.prisma.jobStop.update({ where: { id: stop.id }, data: { arrivedAt: new Date() } });
    await this.notifications.notify({
      userId: updated.customerId,
      event: NotificationEvent.PARTNER_ARRIVED,
      jobId,
      vars: { partnerName: job.partner?.user.fullName ?? '' },
      priority: 'high',
      collapseKey: `job:${jobId}`,
    });
    const waitSeconds = await this.sysConfig.getNumber(CONFIG_KEYS.JOB_WAITING_CUSTOMER_TIMEOUT_S);
    await this.queue.add(
      JOB_JOBS.WAITING_CUSTOMER_TIMEOUT,
      { jobId },
      { delay: waitSeconds * 1000, jobId: `wait-${jobId}` },
    );
    return this.jobs.toDto(await this.jobs.getRaw(jobId), user);
  }
  ...
  async adminArrive(
    ...
    const updated = await this.jobs.transition(
      ...
      { reason, requestId, eventType: 'partner.arrived_override' },
    );
    await this.notifications.notify({
      userId: updated.customerId,
      event: NotificationEvent.PARTNER_ARRIVED,
      jobId,
      vars: { partnerName: updated.partner?.user.fullName ?? '' },
      priority: 'high',
    });
  ...
  async start(
    ...
    if (job.type === JobType.RIDE && job.tripPinRequired) {
      if (
        !input.tripPin ||
        !job.tripPinHash ||
        !safeEqual(job.tripPinHash, hmacHash(input.tripPin, pepper))
      )
        throw AppException.badRequest(ErrorCode.TRIP_PIN_INVALID, 'Invalid trip PIN');
    }
    if (job.type === JobType.DELIVERY && job.pickupOtpRequired) {
      if (
        !input.pickupOtp ||
        !job.pickupOtpHash ||
        !safeEqual(job.pickupOtpHash, hmacHash(input.pickupOtp, pepper))
      )
        throw AppException.badRequest(ErrorCode.PICKUP_OTP_INVALID, 'Invalid pickup code');
    }
    const target =
      job.type === JobType.HOME_SERVICE ? JobStatus.INSPECTION_STARTED : JobStatus.IN_PROGRESS;
    const updated = await this.jobs.transition(
      jobId,
      target,
      { type: JobActorType.PARTNER, id: user.id },
    ...
    if (job.type === JobType.DELIVERY)
      await this.prisma.jobDeliveryDetails.update({
        where: { jobId },
        data: {
          pickupVerifiedAt: new Date(),
          pickupVerifiedMethod: job.pickupOtpRequired ? 'OTP' : 'MANUAL',
        },
      });
    const first = job.stops[0];
    if (first && !first.completedAt)
  ...
  private async maybeSkipInspection(job: JobWithRelations, user: RequestUser): Promise<void> {
    ...
    if (wf && wf.requiresQuote === false) {
      await this.jobs.transition(
        ...
        { type: JobActorType.PARTNER, id: user.id },
        { eventType: 'work.started', data: { autoFromInspection: true } },
      );
    } else {
      await this.jobs.transition(
        job.id,
  ...
  async complete(
    ...
    const job = await this.assertAssignedPartner(jobId, user);
    if (job.type === JobType.HOME_SERVICE)
      throw AppException.invalidTransition(job.status, JobStatus.COMPLETED);
    const pepper = this.config.env.OTP_PEPPER;
    ...
    if (job.type === JobType.DELIVERY) {
      ...
      if (!pod)
        throw AppException.validation([
          { field: 'proofOfDelivery', message: 'required for deliveries' },
        ]);
      ...
      if (job.deliveryOtpRequired) {
        if (
          !pod.deliveryOtp ||
          !job.deliveryOtpHash ||
          !safeEqual(job.deliveryOtpHash, hmacHash(pod.deliveryOtp, pepper))
        )
          throw AppException.badRequest(ErrorCode.DELIVERY_OTP_INVALID, 'Invalid delivery code');
        otpVerified = true;
      } else if (!pod.photoMediaId && !pod.receiverName) {
        throw AppException.validation([
          { field: 'proofOfDelivery', message: 'photo or receiver name required' },
        ]);
      }
      const mediaIds = [pod.photoMediaId, pod.signatureMediaId].filter((m): m is string => !!m);
      if (mediaIds.length)
        await this.media.assertOwnedReady(user.id, mediaIds, [MediaPurpose.PROOF_OF_DELIVERY]);
      podPatch = {
        podReceiverName: pod.receiverName ?? null,
        podPhoto: pod.photoMediaId ? { connect: { id: pod.photoMediaId } } : undefined,
        podSignature: pod.signatureMediaId ? { connect: { id: pod.signatureMediaId } } : undefined,
        podLat: input.location ? new Prisma.Decimal(input.location.lat) : null,
        podLng: input.location ? new Prisma.Decimal(input.location.lng) : null,
        podOtpVerified: otpVerified,
    ...
    const waitingSeconds =
      job.partnerArrivedAt && job.startedAt
        ? Math.max(0, Math.round((job.startedAt.getTime() - job.partnerArrivedAt.getTime()) / 1000))
        : 0;
    const fare = await this.pricing.finalizeFare(job, {
      distanceMeters: actuals.distanceMeters,
      durationSeconds: actuals.durationSeconds,
      waitingSeconds,
    });
    ...
    const updated = await this.jobs.transition(
      ...
      { type: JobActorType.PARTNER, id: user.id },
      {
        ...
        location: input.location,
        data: {
          actualDistanceMeters: actuals.distanceMeters,
          actualDurationSeconds: actuals.durationSeconds,
          finalTotalMinor: Number(fare.totalMinor),
        },
        patch: {
          finalTotalMinor: fare.totalMinor,
          breakdown: fare.breakdown as unknown as Prisma.InputJsonValue,
          actualDistanceMeters: actuals.distanceMeters,
          actualDurationSeconds: actuals.durationSeconds,
        },
      },
    ...
    if (last)
      await this.prisma.jobStop.update({
        where: { id: last.id },
        data: { arrivedAt: last.arrivedAt ?? new Date(), completedAt: new Date() },
      });
    ...
    if (job.status === JobStatus.QUOTE_APPROVED) {
      ...
      if (!quote || quote.status !== 'APPROVED')
        throw AppException.badRequest(
          ...
          'Work requires an approved quote',
        );
    }
    const updated = await this.jobs.transition(
      jobId,
      JobStatus.WORK_STARTED,
      { type: JobActorType.PARTNER, id: user.id },
      { expectedVersion: input.version, requestId, reason: input.note },
    );
  ...
  async resumeWork(
    ...
    return this.jobs.toDto(
      await this.jobs.transition(
        ...
        JobStatus.WORK_STARTED,
        { type: JobActorType.PARTNER, id: user.id },
        { expectedVersion: input.version, requestId, eventType: 'work.resumed' },
      ),
      user,
    );
  }
  ...
  async completeWork(
    ...
    const job = await this.assertAssignedPartner(jobId, user);
    if (job.type !== JobType.HOME_SERVICE)
      throw AppException.invalidTransition(job.status, JobStatus.WORK_COMPLETED);
    const updated = await this.jobs.transition(
      jobId,
      JobStatus.WORK_COMPLETED,
      { type: JobActorType.PARTNER, id: user.id },
      { expectedVersion: input.version, requestId, location: input.location },
    );
    const wf =
      (job.category?.workflowConfig as {
        requiresCustomerConfirmation?: boolean;
        autoConfirmHours?: number;
      } | null) ?? null;
    const hours =
      wf?.autoConfirmHours ?? (await this.sysConfig.getNumber(CONFIG_KEYS.JOB_AUTO_CONFIRM_HOURS));
    if (wf && wf.requiresCustomerConfirmation === false) {
      await this.confirmWorkInternal(
        jobId,
        { type: JobActorType.SYSTEM, id: null },
        'auto_no_confirmation_required',
      );
    } else {
      await this.notifications.notify({
        userId: updated.customerId,
        ...
        jobId,
        vars: { jobNumber: updated.number, total: '' },
        priority: 'high',
      });
      await this.queue.add(
        JOB_JOBS.AUTO_CONFIRM_WORK,
        { jobId },
        { delay: hours * 3600 * 1000, jobId: `autoconfirm-${jobId}` },
      );
    }
    return this.jobs.toDto(await this.jobs.getRaw(jobId), user);
  }
  ...
  async confirmWork(
    ...
    await this.confirmWorkInternal(
      jobId,
      { type: JobActorType.CUSTOMER, id: user.id },
      input.note,
      input.version,
      requestId,
    );
  ...
  async confirmWorkInternal(
    ...
    const confirmed = await this.jobs.transition(jobId, JobStatus.CUSTOMER_CONFIRMED, actor, {
      expectedVersion,
      requestId,
      reason: note,
    });
    ...
    const approvedQuote =
      quote && quote.status === 'APPROVED'
        ? {
            laborMinor: quote.laborCostMinor,
            partsMinor: quote.partsCostMinor,
            feesMinor: quote.additionalFeesMinor,
            discountMinor: quote.discountMinor,
          }
        : null;
    const optionsMinor = confirmed.serviceOptions.reduce((s, o) => s + o.priceMinor, 0n);
    const hours = confirmed.startedAt
      ? Math.max(1, Math.ceil((Date.now() - confirmed.startedAt.getTime()) / 3_600_000))
      : 1;
    const fare = await this.pricing.finalizeFare(confirmed, {
      quote: approvedQuote,
      optionsMinor,
      hours,
    });
    const completed = await this.jobs.transition(
      ...
      { type: JobActorType.SYSTEM, id: null },
      {
        data: { finalTotalMinor: Number(fare.totalMinor) },
        patch: {
          finalTotalMinor: fare.totalMinor,
          breakdown: fare.breakdown as unknown as Prisma.InputJsonValue,
        },
    ...
    if (to === JobStatus.CANCELLED) {
      ...
      const updated = await this.jobs.transition(
        ...
        { type: JobActorType.ADMIN, id: actor.id },
        {
          ...
          requestId,
          patch: {
            cancellationReasonCode: 'ADMIN',
            cancellationReasonText: reason,
            cancelledBy: JobActorType.ADMIN,
            finalTotalMinor: 0n,
          },
        },
      );
  ...
  private async assertAssignedPartner(jobId: string, user: RequestUser): Promise<JobWithRelations> {
    ...
    if (!JobPolicy.isAssignedPartner(user, job))
      throw AppException.forbidden('You are not assigned to this job');
    return job;
  }
}
