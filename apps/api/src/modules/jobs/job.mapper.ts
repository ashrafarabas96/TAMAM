...
import { type FareBreakdownLine, type JobDto, type JobEventDto, type JobStopDto, type Money, type QuoteDto, UserRole } from '@tamam/shared-types';
...
export class JobMapper {
  ...
  toDto(job: JobWithRelations, viewer: Viewer, events?: Array<{ id: string; type: string; fromStatus: JobDto['status'] | null; toStatus: JobDto['status'] | null; actorType: JobEventDto['actorType']; actorId: string | null; data: unknown; createdAt: Date }>): JobDto {
    ...
    const money = (v: bigint | null): Money | null => (v === null ? null : { amount: Number(v), currency });
    ...
    const active = !['COMPLETED', 'CANCELLED', 'NO_PARTNER_AVAILABLE', 'DISPUTED', 'DRAFT', 'REQUESTED', 'SEARCHING'].includes(job.status);
    ...
    return {
      ...
      stops: job.stops.map((s): JobStopDto => ({
        ...
        sequence: s.sequence,
        kind: s.kind,
        address: { lat: s.lat.toNumber(), lng: s.lng.toNumber(), formatted: s.formatted, street: s.street ?? undefined, building: s.building ?? undefined, floor: s.floor ?? undefined, apartment: s.apartment ?? undefined, city: s.city ?? undefined, notes: s.notes ?? undefined, placeId: s.placeId ?? undefined },
      ...
      ...(isCustomer && job.deliveryOtpEnc ? { deliveryOtp: this.safeDecrypt(job.deliveryOtpEnc, key) } : {}),
      ...(isCustomer && job.pickupOtpEnc ? { pickupOtp: this.safeDecrypt(job.pickupOtpEnc, key) } : {}),
      delivery: job.delivery && !isShare
        ? {
            ...
            proof: job.delivery.podTimestamp
              ? { receiverName: job.delivery.podReceiverName, photoUrl: job.delivery.podPhoto ? this.mediaUrls.urlFor(job.delivery.podPhoto, 'medium') : null, signatureUrl: job.delivery.podSignature ? this.mediaUrls.urlFor(job.delivery.podSignature, 'original') : null, location: job.delivery.podLat && job.delivery.podLng ? { lat: job.delivery.podLat.toNumber(), lng: job.delivery.podLng.toNumber() } : null, otpVerified: job.delivery.podOtpVerified, timestamp: job.delivery.podTimestamp.toISOString() }
              : null,
          }
        : undefined,
      partner: partnerUser && job.partner
        ? {
            id: job.partner.userId,
            fullName: partnerUser.fullName ?? '',
            profileImageUrl: partnerUser.profileImage ? this.mediaUrls.urlFor(partnerUser.profileImage, 'thumbnail') : null,
            ...
            maskedPhone: isShare ? null : isStaff ? partnerUser.phone : maskPhone(partnerUser.phone),
            vehicle: job.vehicle ? { brand: job.vehicle.brand, model: job.vehicle.model, color: job.vehicle.color, plate: job.vehicle.plate, typeName: { ar: job.vehicle.vehicleType.nameAr, en: job.vehicle.vehicleType.nameEn } } : null,
            location: active && partnerLive?.lat && partnerLive.lng ? { lat: partnerLive.lat.toNumber(), lng: partnerLive.lng.toNumber() } : null,
          }
        : undefined,
      customer: isShare
        ? undefined
        : {
            ...
            profileImageUrl: job.customer.user.profileImage ? this.mediaUrls.urlFor(job.customer.user.profileImage, 'thumbnail') : null,
            rating: job.customer.ratingCount ? Number((job.customer.ratingSum / job.customer.ratingCount).toFixed(2)) : 5,
            maskedPhone: isPartner || isStaff ? (isStaff ? job.customer.user.phone : maskPhone(job.customer.user.phone)) : null,
          },
      ...
      events: events?.map((e) => ({ id: e.id, type: e.type, fromStatus: e.fromStatus, toStatus: e.toStatus, actorType: e.actorType, actorId: isStaff ? e.actorId : null, data: isStaff ? ((e.data as Record<string, unknown> | null) ?? null) : null, createdAt: e.createdAt.toISOString() })),
    };
  }

  quoteToDto(q: Prisma.ServiceQuoteGetPayload<{ include: { items: true } }>, currency: Money['currency']): QuoteDto {
    ...
    return {
      ...
      items: q.items.map((i) => ({ id: i.id, kind: i.kind, description: i.description, quantity: i.quantity.toNumber(), unitPrice: m(i.unitPriceMinor), total: m(i.totalMinor) })),
