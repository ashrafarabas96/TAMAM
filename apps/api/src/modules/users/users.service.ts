...
export class UsersService {
  ...
  async createFromPhone(
    ...
    const referredBy = referralCode
      ? await tx.customerProfile.findUnique({ where: { referralCode: referralCode.toUpperCase() } })
      : null;
    const user = await tx.user.create({
      data: {
        ...
        language,
        roles: {
          create: [{ role: audience === 'PARTNER' ? UserRole.PARTNER : UserRole.CUSTOMER }],
        },
        customer:
          audience === 'CUSTOMER'
            ? {
                create: {
                  ...
                  referredById: referredBy?.userId ?? null,
                },
              }
            : undefined,
    ...
    if (audience === 'CUSTOMER') {
      ...
      if (!exists)
        await tx.customerProfile.create({
          data: { userId, referralCode: await this.uniqueReferralCode(tx) },
        });
    } else {
      const exists = await tx.partnerProfile.findUnique({ where: { userId } });
      if (!exists)
        await tx.partnerProfile.create({ data: { userId, availability: { create: {} } } });
    }
  }
  ...
  async updateProfile(userId: string, input: UpdateProfileInput): Promise<UserDto> {
    ...
    const user = await this.prisma.user.update({
      ...
      data: {
        ...
        profileImageId: input.profileImageMediaId,
      },
      include: userInclude,
    });
    return this.toDto(user);
  ...
  async changeAccountStatus(
    ...
    if (target.roles.some((r) => r.role === UserRole.SUPER_ADMIN))
      throw AppException.forbidden('Cannot change status of a super admin');
    ...
    const user = await this.prisma.$transaction(async (tx) => {
      const updated = await tx.user.update({
        ...
        data: {
          ...
          deletedAt: next === AccountStatus.DELETED ? new Date() : null,
        },
        include: userInclude,
      });
      if (next !== AccountStatus.ACTIVE) {
        await tx.userSession.updateMany({
          ...
          data: { revokedAt: new Date(), revokedReason: `account_${next.toLowerCase()}` },
        });
      }
      await this.audit.record(
        {
          ...
          newValue: { accountStatus: next, until: input.until ?? null },
          reason: input.reason,
          requestId,
        },
        tx,
  ...
  toDto(user: {
    ...
    return {
      ...
      customer: customer
        ? {
            rating: customer.ratingCount
              ? Number((customer.ratingSum / customer.ratingCount).toFixed(2))
              : 5,
            ratingCount: customer.ratingCount,
            completedJobs: customer.completedJobs,
            cancelledJobs: customer.cancelledJobs,
      ...
      partner: partner
        ? {
            id: partner.userId,
            verificationStatus: partner.verificationStatus as UserDto['partner'] extends infer P
              ? P extends { verificationStatus: infer V }
                ? V
                : never
              : never,
            availability: (partner.availability?.status ?? 'OFFLINE') as NonNullable<
              UserDto['partner']
            >['availability'],
            roles: partner.roles
              .filter((r) => r.isActive)
              .map((r) => r.role as NonNullable<UserDto['partner']>['roles'][number]),
            rating: partner.ratingCount
              ? Number((partner.ratingSum / partner.ratingCount).toFixed(2))
              : 5,
            ratingCount: partner.ratingCount,
            completedJobs: partner.completedJobs,
            acceptanceRate: partner.offersReceived
              ? Number((partner.offersAccepted / partner.offersReceived).toFixed(3))
              : 1,
            cancellationRate:
              partner.completedJobs + partner.cancelledJobs
                ? Number(
                    (
                      partner.cancelledJobs /
                      (partner.completedJobs + partner.cancelledJobs)
                    ).toFixed(3),
                  )
                : 0,
