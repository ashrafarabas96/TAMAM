...
export class QuotesService {
  ...
  async submit(
    ...
    if (!JobPolicy.isAssignedPartner(user, job))
      throw AppException.forbidden('Only the assigned technician can quote');
    if (job.type !== JobType.HOME_SERVICE)
      throw AppException.invalidTransition(job.status, JobStatus.QUOTE_SUBMITTED);
    const isChangeOrder = input.kind === 'CHANGE_ORDER' || job.status === JobStatus.WORK_STARTED;
    if (
      isChangeOrder &&
      job.status !== JobStatus.WORK_STARTED &&
      job.status !== JobStatus.WAITING_FOR_PARTS
    )
      throw AppException.invalidTransition(job.status, JobStatus.QUOTE_SUBMITTED);
    if (
      !isChangeOrder &&
      job.status !== JobStatus.QUOTE_REQUIRED &&
      job.status !== JobStatus.QUOTE_REJECTED &&
      job.status !== JobStatus.INSPECTION_STARTED
    )
      throw AppException.invalidTransition(job.status, JobStatus.QUOTE_SUBMITTED);

    const rule = job.pricingSnapshot
      ? await this.prisma.pricingSnapshot.findUnique({
          where: { id: job.pricingSnapshot.id },
          select: { rule: true },
        })
      : null;
    ...
    if (net < 0n)
      throw AppException.validation([
        { field: 'discountMinor', message: 'discount exceeds quote' },
      ]);
    const tax = taxPercent > 0 ? percentOf(net, taxPercent) : 0n;
    const total = net + tax;

    const quote = await this.prisma.$transaction(async (tx) => {
      const previous = await tx.serviceQuote.findFirst({
        where: { jobId, status: { in: [QuoteStatus.SUBMITTED, QuoteStatus.REJECTED] } },
        orderBy: { revision: 'desc' },
      });
      const lastRevision = await tx.serviceQuote.aggregate({
        where: { jobId },
        _max: { revision: true },
      });
      if (previous && previous.status === QuoteStatus.SUBMITTED)
      ...
      const created = await tx.serviceQuote.create({
        data: {
          ...
          kind: isChangeOrder ? QuoteKind.CHANGE_ORDER : QuoteKind.INITIAL,
      ...
      await this.jobs
        .transitionInTx(
          ...
          { type: JobActorType.PARTNER, id: user.id },
          {
            ...
            requestId,
            eventType: isChangeOrder ? 'change_order.submitted' : 'quote.submitted',
            data: { quoteId: created.id, revision: created.revision, totalMinor: Number(total) },
          },
        )
        .then((t) => t.emit());
      return created;
    ...
    await this.queue.add(
      JOB_JOBS.QUOTE_RESPONSE_TIMEOUT,
      { jobId, quoteId: quote.id },
      { delay: timeoutHours * 3600 * 1000, jobId: `quote-timeout-${quote.id}` },
    );
    await this.notifications.notify({
      ...
      vars: {
        ...
        total: formatMajor(total, job.currency),
      },
  ...
  async decide(
    ...
    if (job.status !== JobStatus.QUOTE_SUBMITTED)
      throw AppException.invalidTransition(
        job.status,
        input.decision === 'APPROVE' ? JobStatus.QUOTE_APPROVED : JobStatus.QUOTE_REJECTED,
      );
    const quote = await this.prisma.serviceQuote.findFirst({
      where: { jobId, status: QuoteStatus.SUBMITTED },
      include: { items: true },
      orderBy: { revision: 'desc' },
    });
    ...
    const updated = await this.prisma.$transaction(async (tx) => {
      const q = await tx.serviceQuote.update({
        where: { id: quote.id },
        data: {
          status: approve ? QuoteStatus.APPROVED : QuoteStatus.REJECTED,
          decidedAt: new Date(),
          decidedById: user.id,
      ...
      if (approve && isChangeOrder) {
        ...
        await tx.serviceQuote.updateMany({
          where: { jobId, status: QuoteStatus.APPROVED, id: { not: q.id } },
          data: { status: QuoteStatus.SUPERSEDED },
        });
      }
      const target = approve ? JobStatus.QUOTE_APPROVED : JobStatus.QUOTE_REJECTED;
      const t = await this.jobs.transitionInTx(tx, jobId, target, actor, {
        expectedVersion: input.version,
        requestId,
        reason: input.note,
        data: { quoteId: q.id, revision: q.revision },
      });
      if (approve && isChangeOrder) {
        // Work was already in progress: go straight back to WORK_STARTED.
        const t2 = await this.jobs.transitionInTx(
          tx,
          jobId,
          JobStatus.WORK_STARTED,
          { type: JobActorType.PARTNER, id: job.partnerId },
          { eventType: 'work.resumed_after_change_order', data: { quoteId: q.id } },
        );
        t.emit();
        t2.emit();
      } else {
      ...
      if (approve)
        await tx.job.update({ where: { id: jobId }, data: { estimatedTotalMinor: q.totalMinor } });
      return q;
    });
    ...
    if (job.partnerId) {
      await this.notifications.notify({
        ...
        event: approve ? NotificationEvent.QUOTE_APPROVED : NotificationEvent.QUOTE_REJECTED,
        jobId,
        priority: 'high',
        vars: { total: formatMajor(updated.totalMinor, job.currency) },
      });
    }
    this.events.emit('quote.decided', {
      jobId,
      quoteId: updated.id,
  ...
  async onResponseTimeout(quoteId: string): Promise<void> {
    ...
    await this.jobs.transition(
      ...
      { eventType: 'quote.timeout', data: { quoteId } },
    );
    if (job.partnerId)
      await this.notifications.notify({
        userId: job.partnerId,
        event: NotificationEvent.QUOTE_REJECTED,
        jobId: job.id,
        vars: {},
      });
  }
  ...
  async closeInspectionOnly(
    ...
    const job = await this.jobs.getForUser(jobId, user);
    if (!JobPolicy.isCustomer(user, job) && !JobPolicy.isStaff(user))
      throw AppException.forbidden();
    if (job.status !== JobStatus.QUOTE_REJECTED)
    ...
    await this.jobs.transition(
      ...
      { type: JobPolicy.actorTypeFor(user, job), id: user.id },
      {
        ...
        requestId,
        eventType: 'job.completed_inspection_only',
        patch: {
