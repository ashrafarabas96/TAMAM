...
/// Phone + OTP sign-in for the PARTNER audience.
///
/// The audience is what makes the server attach (or create) a partner profile
/// to the phone number instead of a customer account.
class AuthRepository {
  const AuthRepository(this._api);

  static const String audience = 'PARTNER';
  ...
  Future<OtpChallenge> requestOtp({required String phone, required String language}) async =>
      OtpChallenge.fromJson(
        await _api.postObject(
          ApiPaths.otpRequest,
          body: <String, Object?>{'phone': phone, 'audience': audience, 'language': language},
        ),
      );
