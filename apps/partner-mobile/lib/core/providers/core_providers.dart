...
import 'package:tamam_partner/core/config/feature_flags.dart';
...
import 'package:tamam_partner/core/network/connectivity_service.dart';
import 'package:tamam_partner/core/network/interceptors.dart';
import 'package:tamam_partner/core/network/token_refresher.dart';
import 'package:tamam_partner/core/session/session_controller.dart';
...
import 'package:tamam_partner/core/session/session_teardown.dart';
...
/// Everything that must stop when the session ends (location, foreground
/// service, sockets) registers itself here.
final Provider<SessionTeardown> sessionTeardownProvider =
    Provider<SessionTeardown>((Ref ref) => SessionTeardown());
...
/// Battery + network type for the heartbeat body.
final Provider<DeviceMetricsReader> deviceMetricsReaderProvider =
    Provider<DeviceMetricsReader>((Ref ref) => DeviceMetricsReader());
...
/// The signed-in partner's user id, or `null` — used by chat to tell messages apart.
  ...
  Future<FeatureFlags> build() async {
    ...
    try {
      final FeatureFlags flags =
          FeatureFlags.fromJson(await ref.read(apiClientProvider).getObject(ApiPaths.featureFlags));
