...
function emitDart(targetDir) {
  ...
  lines.push(``);
  lines.push(`  /// The face these styles render in when a call site does not name one.`);
  lines.push(`  ///`);
  lines.push(`  /// Most styles in the app are built by calling toTextStyle() directly rather`);
  lines.push(`  /// than by reading Theme.of(context).textTheme, so setting the family on the`);
  lines.push(`  /// TextTheme alone left those call sites with a null family and the platform`);
  lines.push(`  /// default -- the app rendered in two typefaces at once. Defaulting it here`);
  lines.push(`  /// covers every call site, themed or not.`);
  lines.push(`  static String get family =>`);
  lines.push(`      _isArabic ? TamamFonts.arabic : TamamFonts.latin;`);
  lines.push(``);
  ...
  lines.push(`        fontFamily: fontFamily ?? family,`);
  lines.push(`        fontFamilyFallback: fontFamily != null`);
  lines.push(`            ? null`);
  lines.push(`            : (_isArabic`);
  lines.push(`                ? <String>[TamamFonts.latin, ...TamamFonts.fallbackArabic]`);
  lines.push(`                : <String>[TamamFonts.arabic, ...TamamFonts.fallbackLatin]),`);
