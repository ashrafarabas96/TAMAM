...
describe('bannerSchema', () => {
  const creative = {
    imageMediaId: {
      ar: '2c9f7a3e-1a4b-4c3d-9e8f-0a1b2c3d4e5f',
      en: '2c9f7a3e-1a4b-4c3d-9e8f-0a1b2c3d4e5f',
    },
  };
...
describe('updateConfigSchema', () => {
  it('rejects out of range values', () => {
    ...
    expect(
      updateConfigSchema.safeParse({ key: 'commission.default_percent', value: 20, reason: 'test' })
        .success,
    ).toBe(true);
  });
  it('rejects unknown keys', () => {
    expect(updateConfigSchema.safeParse({ key: 'nope', value: 1, reason: 'test' }).success).toBe(
      false,
    );
  });
});
